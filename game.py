...
from game_settings import (
    ...
    GAME_STATE_BONUS_LEVEL_TRANSITION, GAME_STATE_BONUS_LEVEL_START, GAME_STATE_BONUS_LEVEL_PLAYING,
...
class CoreFragmentItem(Collectible):
    ...
    def apply_effect(self, player, game_instance):
...
class Game:
    def __init__(self):
        ...
        self.player_spawn_check_dimension = 40
        ...
        self.font_size_ui_text = 28; self.font_size_ui_values = 30; self.font_size_ui_emoji_general = 32
        self.font_size_ui_emoji_small = 20; self.font_size_small_text = 24; self.font_size_medium_text = 48
        self.font_size_large_text = 74; self.font_size_input_text = 50; self.font_size_menu_text = 60
        self.font_size_title_text = 90; self.font_size_drone_name_grid = 36; self.font_size_drone_desc_grid = 22
        self.font_size_drone_unlock_grid = 20; self.font_size_drone_name_cycle = 42
        self.font_size_drone_stats_label_cycle = 26; self.font_size_drone_stats_value_cycle = 28
        self.font_size_drone_desc_cycle = 22; self.font_size_drone_unlock_cycle = 20
        try:
            self.font_ui_emoji_general = pygame.font.Font(self.font_path_emoji, self.font_size_ui_emoji_general)
            self.font_ui_emoji_small = pygame.font.Font(self.font_path_emoji, self.font_size_ui_emoji_small)
            self.font_ui_text = pygame.font.Font(self.font_path_neuropol, self.font_size_ui_text)
            self.font_ui_values = pygame.font.Font(self.font_path_neuropol, self.font_size_ui_values)
            self.font_default_text = pygame.font.Font(self.font_path_neuropol, self.font_size_ui_text)
            self.small_text_font = pygame.font.Font(self.font_path_neuropol, self.font_size_small_text)
            self.medium_text_font = pygame.font.Font(self.font_path_neuropol, self.font_size_medium_text)
            self.large_text_font = pygame.font.Font(self.font_path_neuropol, self.font_size_large_text)
            self.input_text_font = pygame.font.Font(self.font_path_neuropol, self.font_size_input_text)
            self.menu_font = pygame.font.Font(self.font_path_neuropol, self.font_size_menu_text)
            self.title_font = pygame.font.Font(self.font_path_neuropol, self.font_size_title_text)
            self.timer_display_font = pygame.font.Font(self.font_path_neuropol, self.font_size_ui_values)
            self.font_drone_name_grid = pygame.font.Font(self.font_path_neuropol, self.font_size_drone_name_grid)
            self.font_drone_desc_grid = pygame.font.Font(self.font_path_neuropol, self.font_size_drone_desc_grid)
            self.font_drone_unlock_grid = pygame.font.Font(self.font_path_neuropol, self.font_size_drone_unlock_grid)
            self.font_drone_name_cycle = pygame.font.Font(self.font_path_neuropol, self.font_size_drone_name_cycle)
            self.font_drone_stats_label_cycle = pygame.font.Font(self.font_path_neuropol, self.font_size_drone_stats_label_cycle)
            self.font_drone_stats_value_cycle = pygame.font.Font(self.font_path_neuropol, self.font_size_drone_stats_value_cycle)
            self.font_drone_desc_cycle = pygame.font.Font(self.font_path_neuropol, self.font_size_drone_desc_cycle)
            self.font_drone_unlock_cycle = pygame.font.Font(self.font_path_neuropol, self.font_size_drone_unlock_cycle)
        except pygame.error as e:
            print(f"Warning: Font loading error: {e}. Using fallbacks.")
            self.font_ui_emoji_general=pygame.font.Font(None,self.font_size_ui_emoji_general); self.font_ui_emoji_small=pygame.font.Font(None,self.font_size_ui_emoji_small)
            self.font_ui_text=pygame.font.Font(None,self.font_size_ui_text); self.font_ui_values=pygame.font.Font(None,self.font_size_ui_values)
            self.font_default_text=pygame.font.Font(None,self.font_size_ui_text); self.small_text_font=pygame.font.Font(None,self.font_size_small_text)
            self.medium_text_font=pygame.font.Font(None,self.font_size_medium_text); self.large_text_font=pygame.font.Font(None,self.font_size_large_text)
            self.input_text_font=pygame.font.Font(None,self.font_size_input_text); self.menu_font=pygame.font.Font(None,self.font_size_menu_text)
            self.title_font=pygame.font.Font(None,self.font_size_title_text); self.timer_display_font=pygame.font.Font(None,self.font_size_ui_values)
            self.font_drone_name_grid=pygame.font.Font(None,self.font_size_drone_name_grid); self.font_drone_desc_grid=pygame.font.Font(None,self.font_size_drone_desc_grid)
            self.font_drone_unlock_grid=pygame.font.Font(None,self.font_size_drone_unlock_grid); self.font_drone_name_cycle=pygame.font.Font(None,self.font_size_drone_name_cycle)
            self.font_drone_stats_label_cycle=pygame.font.Font(None,self.font_size_drone_stats_label_cycle); self.font_drone_stats_value_cycle=pygame.font.Font(None,self.font_size_drone_stats_value_cycle)
            self.font_drone_desc_cycle=pygame.font.Font(None,self.font_size_drone_desc_cycle); self.font_drone_unlock_cycle=pygame.font.Font(None,self.font_size_drone_unlock_cycle)

        self.font = self.font_default_text; self.small_font = self.small_text_font
        self.medium_font = self.medium_text_font; self.large_font = self.large_text_font
        self.input_font = self.input_text_font
        ...
        self.drone_select_options = DRONE_DISPLAY_ORDER; self.selected_drone_preview_index = 0
        ...
        self.played_bonus_level_after_fragments = False; self.current_bonus_level_objective = None
        self.bonus_level_timer_start = 0; self.bonus_level_duration_ms = 60000
        ...
        if drone_config and drone_config.get("icon_path"):
            try:
                ...
                self.current_drone_life_icon_surface = self._create_fallback_icon(
                    size=self.ui_icon_size_lives, text="♥", color=CYAN, font_to_use=self.font_ui_emoji_small
                )
        else:
            self.current_drone_life_icon_surface = self._create_fallback_icon(
                size=self.ui_icon_size_lives, text="♥", color=CYAN, font_to_use=self.font_ui_emoji_small
    ...
    def _create_fallback_icon(self, size=(80,80), text="?", color=GREY, text_color=WHITE, font_to_use=None):
        if font_to_use is None:
            font_to_use = self.medium_text_font
        ...
        for drone_id, data in DRONE_DATA.items():
            ...
            if "icon_path" in data and data["icon_path"]:
                try:
                    ...
                    print(f"Error loading grid icon for {drone_id} ('{data['icon_path']}'): {e}. Creating fallback.")
                    initials = data.get("name", "?")[:2].upper()
                    self.drone_grid_icons_cache[drone_id] = self._create_fallback_icon(
                        size=icon_display_size, text=initials, font_to_use=self.font_drone_name_grid
                    )
            else:
                initials = data.get("name", "?")[:2].upper()
                self.drone_grid_icons_cache[drone_id] = self._create_fallback_icon(
    ...
    def _load_drone_main_display_images(self):
        ...
        for drone_id, data in DRONE_DATA.items():
            image_surface = None
            main_display_path_key = "sprite_path" if "sprite_path" in data and data["sprite_path"] else "icon_path"
            if data.get(main_display_path_key) and isinstance(data[main_display_path_key], str):
                try:
                    if os.path.exists(data[main_display_path_key]):
                        loaded_image = pygame.image.load(data[main_display_path_key]).convert_alpha()
                        image_surface = pygame.transform.smoothscale(loaded_image, display_size)
                except pygame.error as e:
                    print(f"Error loading main display image for {drone_id} ('{data[main_display_path_key]}'): {e}.")
            if image_surface is None:
                initials = data.get("name", "?")[:2].upper()
                image_surface = self._create_fallback_icon(
                    size=display_size, text=initials, font_to_use=self.large_text_font
                )
    ...
    def load_sfx(self):
        sound_paths = {'collect_ring':"assets/sounds/collect_ring.wav",'weapon_upgrade_collect':"assets/sounds/collect_powerup.wav",
    ...
    def set_game_state(self, new_state):
        if self.game_state == new_state and not (new_state == GAME_STATE_PLAYING and self.paused) \
           and not (new_state == GAME_STATE_BONUS_LEVEL_PLAYING and self.paused):
            return
        self.game_state = new_state
        print(f"Game state changed to: {self.game_state}")
        music_map = {GAME_STATE_MAIN_MENU:"menu",GAME_STATE_PLAYING:"gameplay",GAME_STATE_LEADERBOARD:"menu",
                     GAME_STATE_ENTER_NAME:"menu",GAME_STATE_GAME_OVER:"menu",GAME_STATE_SETTINGS:"menu",
                     GAME_STATE_DRONE_SELECT:"menu",GAME_STATE_BONUS_LEVEL_START:"gameplay",
                     GAME_STATE_BONUS_LEVEL_PLAYING:"gameplay",GAME_STATE_BONUS_LEVEL_TRANSITION:"menu"}
        ...
        if target_music_context:
            ...
            if self.current_music_context != target_music_context or not pygame.mixer.music.get_busy():
                self._play_music(music_path_to_play, target_music_context)
            elif self.game_state in [GAME_STATE_PLAYING, GAME_STATE_BONUS_LEVEL_PLAYING] and self.paused:
                 pygame.mixer.music.unpause()
        if self.game_state == GAME_STATE_BONUS_LEVEL_START:
            self.initialize_bonus_level_session()
    ...
    def initialize_game_session(self):
        ...
        if new_unlocks_level: print(f"New drones unlocked at L1: {new_unlocks_level}")
        self.level_cleared_pending_animation = False; self.all_enemies_killed_this_level = False
        self.played_bonus_level_after_fragments = False
    ...
    def initialize_bonus_level_session(self):
        ...
        self.enemies.empty(); self.rings.empty(); self.power_ups.empty(); self.core_fragments.empty()
        self.current_bonus_level_objective = "survive_timer"
    ...
    def get_safe_spawn_point(self, drone_check_width, drone_check_height):
        ...
        if not path_cells_relative: print("ERROR: No path cells found in maze for spawning!"); return (WIDTH // 4 + self.UI_PANEL_WIDTH, GAME_PLAY_AREA_HEIGHT // 2)
    ...
    def spawn_enemies(self):
        ...
        for _ in range(num_enemies):
            ...
            while spawn_attempts < 10 and not spawned:
                ...
                if self.player and math.hypot(abs_x-self.player.x,abs_y-self.player.y) < TILE_SIZE*5: spawn_attempts+=1; continue
                if any(math.hypot(abs_x-e.x,abs_y-e.y) < TILE_SIZE*2 for e in self.enemies): spawn_attempts+=1; continue
    ...
    def draw_main_menu(self):
        ...
        for i, option_text in enumerate(self.menu_options):
            ...
            active_menu_font = self.menu_font
            if is_selected:
                 try: active_menu_font = pygame.font.Font(self.font_path_neuropol, self.font_size_menu_text + 8)
                 except: active_menu_font = pygame.font.Font(None, self.font_size_menu_text + 8)
            text_surf = active_menu_font.render(option_text, True, text_color); text_rect = text_surf.get_rect()
            button_width = text_rect.width + 60; button_height = text_rect.height + 25
            button_surface_rect = pygame.Rect(0,0,button_width, button_height); button_surface_rect.center = (WIDTH // 2, menu_item_start_y + i * item_spacing)
        ...
        instr_surf = self.small_text_font.render("Use UP/DOWN keys, ENTER to select.", True, CYAN)
        ...
        if get_game_setting("SETTINGS_MODIFIED"):
            warning_surf = self.small_text_font.render("Custom settings active: Leaderboard disabled.", True, YELLOW)
    ...
    def draw_drone_select_menu(self):
        ...
        title_rect = title_surf.get_rect(center=(WIDTH // 2, 70)); self.screen.blit(title_surf, title_rect)
        num_options = len(self.drone_select_options)
        if not self.drone_select_options:
            no_drones_surf = self.large_font.render("NO DRONES AVAILABLE", True, RED)
            self.screen.blit(no_drones_surf, no_drones_surf.get_rect(center=(WIDTH//2, HEIGHT//2))); return
        current_drone_id = self.drone_select_options[self.selected_drone_preview_index]
        drone_config = DRONE_DATA.get(current_drone_id, {}); drone_stats = self.drone_system.get_drone_stats(current_drone_id)
        is_unlocked = self.drone_system.is_drone_unlocked(current_drone_id); is_currently_equipped = (current_drone_id == self.drone_system.get_selected_drone_id())
        ...
        name_text = drone_config.get("name", "N/A"); name_surf_temp = self.font_drone_name_cycle.render(name_text, True, WHITE); name_height = name_surf_temp.get_height()
        hp_stat = drone_stats.get("hp"); speed_stat = drone_stats.get("speed"); turn_speed_stat = drone_stats.get("turn_speed")
        fire_rate_mult = drone_stats.get("fire_rate_multiplier", 1.0); special_ability_key = drone_stats.get("special_ability")
        ...
        fire_rate_text = f"{fire_rate_mult:.1f}x";
        ...
        stats_content_surfaces = []; max_stat_label_w = 0; max_stat_value_w = 0; stat_line_h = self.font_drone_stats_label_cycle.get_height() + 5
        for label_str, value_str in stats_data_tuples:
            label_s = self.font_drone_stats_label_cycle.render(label_str, True, LIGHT_BLUE if is_unlocked else GREY)
            value_s = self.font_drone_stats_value_cycle.render(value_str, True, WHITE if is_unlocked else GREY)
        ...
        stats_box_padding = 10; stats_box_visual_width = max_stat_label_w + max_stat_value_w + 3 * stats_box_padding
        stats_box_visual_height = (len(stats_content_surfaces) * stat_line_h) - (5 if stats_content_surfaces else 0) + 2 * stats_box_padding
        desc_text = drone_config.get("description", ""); desc_color_final = (200,200,200) if is_unlocked else (100,100,100)
        desc_max_width_for_card = WIDTH * 0.5; desc_lines_surfs = []; words = desc_text.split(' '); current_line_text_desc = ""
        for word in words:
            test_line = current_line_text_desc + word + " "
            if self.font_drone_desc_cycle.size(test_line)[0] < desc_max_width_for_card: current_line_text_desc = test_line
            else: desc_lines_surfs.append(self.font_drone_desc_cycle.render(current_line_text_desc.strip(), True, desc_color_final)); current_line_text_desc = word + " "
        if current_line_text_desc: desc_lines_surfs.append(self.font_drone_desc_cycle.render(current_line_text_desc.strip(), True, desc_color_final))
        ...
        if not is_unlocked:
            condition_text_str = drone_config.get("unlock_condition",{}).get("description","Locked"); unlock_cost_val = drone_config.get("unlock_condition",{}).get("value")
            type_is_cores_unlock = drone_config.get("unlock_condition",{}).get("type") == "cores"; unlock_text_str = condition_text_str
            if type_is_cores_unlock and unlock_cost_val is not None and self.drone_system.get_player_cores() >= unlock_cost_val: unlock_text_str += f" (ENTER to Unlock: {unlock_cost_val} 💠)"; unlock_text_color = GREEN
        ...
        unlock_info_surf = self.font_drone_unlock_cycle.render(unlock_text_str, True, unlock_text_color)
        ...
        card_w = max_content_width_for_card + 2*padding_inside_card; card_w = min(card_w, WIDTH*0.7)
        card_h = card_content_total_h + 2*padding_inside_card
        main_card_x = display_area_center_x - card_w//2; main_card_y = title_rect.bottom + 30
        main_card_rect = pygame.Rect(main_card_x, main_card_y, card_w, card_h)
        pygame.draw.rect(self.screen, (25,30,40,220), main_card_rect, border_radius=20)
        ...
        if drone_image_surf:
            ...
            self.screen.blit(display_drone_image, final_img_rect); current_y_in_card = final_img_rect.bottom + spacing_between_elements
        else: current_y_in_card += img_height + spacing_between_elements
        name_color_final = WHITE if is_unlocked else GREY; name_surf_final = self.font_drone_name_cycle.render(name_text, True, name_color_final)
        ...
        pygame.draw.rect(self.screen, (40,45,55,180), final_stats_box_draw_rect, border_radius=10)
        ...
        for line_surf in desc_lines_surfs:
            self.screen.blit(line_surf, line_surf.get_rect(centerx=main_card_rect.centerx, top=desc_start_y_render)); desc_start_y_render += line_surf.get_height() + 3
        ...
        arrow_font = self.large_text_font; left_arrow_surf = arrow_font.render("◀", True, WHITE if num_options > 1 else GREY); right_arrow_surf = arrow_font.render("▶", True, WHITE if num_options > 1 else GREY)
        ...
        if num_options > 1:
            ...
            self.screen.blit(left_arrow_surf, left_arrow_rect); self.screen.blit(right_arrow_surf, right_arrow_rect)
        instr_surf = self.small_text_font.render("LEFT/RIGHT: Cycle | ENTER: Select/Unlock | ESC: Back", True, CYAN)
        instr_bg_rect = pygame.Rect(0, HEIGHT-60, WIDTH, 30); instr_surf_rect = instr_surf.get_rect(center=instr_bg_rect.center)
        self.screen.blit(instr_surf, instr_surf_rect)
        cores_label_text_surf = self.font_ui_text.render(f"Player Cores: ", True, GOLD); cores_value_text_surf = self.font_ui_values.render(f"{self.drone_system.get_player_cores()}", True, GOLD)
        cores_emoji_surf = self.font_ui_emoji_general.render(" 💠", True, GOLD)
    ...
    def draw_settings_menu(self): # Definition restored
        self.screen.fill(BLACK)
        if self.menu_stars:
            for star_params in self.menu_stars: pygame.draw.circle(self.screen,WHITE,(int(star_params[0]),int(star_params[1])),star_params[3])
        ...
        item_y_start=180; item_line_height=self.font_default_text.get_height()+15
        max_items_on_screen=(HEIGHT-item_y_start-100)//item_line_height
        view_start_index = max(0,min(self.selected_setting_index-max_items_on_screen+1 if self.selected_setting_index >= max_items_on_screen else 0, len(self.settings_items)-max_items_on_screen if self.settings_items else 0))
        ...
        for i_display, list_idx in enumerate(range(view_start_index, view_end_index)):
            ...
            label_surf = self.font_default_text.render(item["label"], True, color)
            ...
            if "note" in item and list_idx == self.selected_setting_index:
                note_surf = self.small_text_font.render(item["note"], True, LIGHT_BLUE)
            ...
            if item["type"] != "action":
                ...
                value_surf = self.font_default_text.render(display_value, True, color)
                ...
                if item["key"] in DEFAULT_SETTINGS and current_value != DEFAULT_SETTINGS[item["key"]]:
                    self.screen.blit(self.small_text_font.render("*",True,RED),(WIDTH//2+180,y_pos))
            elif list_idx == self.selected_setting_index:
                 action_hint_surf = self.font_default_text.render("<ENTER>", True, YELLOW)
        ...
        instr_surf = self.small_text_font.render("UP/DOWN: Select | LEFT/RIGHT: Adjust | ENTER: Activate | ESC: Back", True, CYAN)
        ...
        if get_game_setting("SETTINGS_MODIFIED"):
            warning_surf = self.small_text_font.render("Settings changed. Leaderboard will be disabled.", True, YELLOW)
    ...
    def handle_events(self): # Definition restored
        current_time = pygame.time.get_ticks()
                ...
                if self.game_state == GAME_STATE_MAIN_MENU:
                    ...
                    if num_options>0:
                        if event.key==pygame.K_LEFT: self.selected_drone_preview_index=(self.selected_drone_preview_index-1+num_options)%num_options; self.play_sound('ui_select')
                            ...
                            if self.drone_system.is_drone_unlocked(selected_id):
                                ...
                                drone_cfg=self.drone_system.get_drone_config(selected_id); unlock_cond=drone_cfg.get("unlock_condition",{})
                                if unlock_cond.get("type")=="cores":
                    ...
                    if event.key == pygame.K_UP: self.selected_setting_index=(self.selected_setting_index-1+len(self.settings_items))%len(self.settings_items); self.play_sound('ui_select')
                        ...
                        if setting_item["type"]!="action":
                            ...
                            if setting_item["type"]=="numeric":
                                ...
                                current_choice_idx=setting_item["choices"].index(current_val); new_choice_idx=(current_choice_idx+direction+len(setting_item["choices"]))%len(setting_item["choices"])
                    ...
                    if event.key == pygame.K_p:
                        ...
                        if self.paused: pygame.mixer.music.pause()
                            ...
                            pygame.mixer.music.unpause()
                            if self.game_state == GAME_STATE_PLAYING: self.level_timer_start_ticks = pygame.time.get_ticks()-(get_game_setting("LEVEL_TIMER_DURATION")-self.level_time_remaining_ms)
                            elif self.game_state == GAME_STATE_BONUS_LEVEL_PLAYING: time_passed_in_bonus = pygame.time.get_ticks()-self.bonus_level_timer_start; self.level_time_remaining_ms = self.bonus_level_duration_ms-time_passed_in_bonus
                    if not self.paused and self.player and self.player.alive:
                        ...
                        if self.game_state==GAME_STATE_BONUS_LEVEL_PLAYING and event.key==pygame.K_ESCAPE: self.paused=False; pygame.mixer.music.unpause(); self.end_bonus_level(completed=False)
        ...
        if (self.game_state == GAME_STATE_PLAYING or self.game_state == GAME_STATE_BONUS_LEVEL_PLAYING) and not self.paused and self.player and self.player.alive:
            keys = pygame.key.get_pressed(); self.player.handle_input(keys, current_time)
            if keys[pygame.K_SPACE]: self.player.shoot(sound=self.sounds.get('shoot'),missile_sound=self.sounds.get('missile_launch'),maze=self.maze,enemies_group=self.enemies if self.game_state == GAME_STATE_PLAYING else None)

    def update_bonus_level(self):
        current_game_ticks = pygame.time.get_ticks()
        if not self.player or not self.player.alive: self.end_bonus_level(completed=False); return
        ...
        if elapsed_bonus_time >= self.bonus_level_duration_ms: self.end_bonus_level(completed=True); return
    ...
    def end_bonus_level(self, completed=True):
        ...
        if self.player:
            ...
            self.player.reset(new_player_pos[0],new_player_pos[1],drone_id=current_drone_id,drone_stats=effective_drone_stats,drone_sprite_path=player_ingame_sprite)
            self.player.reset_active_powerups()
        self.spawn_enemies(); self.core_fragments.empty(); self.place_collectibles(initial_setup=True); self._reset_level_timer()
        self.set_game_state(GAME_STATE_PLAYING)
    ...
    def update(self):
        ...
        if self.game_state == GAME_STATE_PLAYING and not self.paused:
            ...
            if not self.level_cleared_pending_animation:
                ...
                for enemy_obj in list(self.enemies):
                    ...
                    if not enemy_obj.alive and not enemy_obj.bullets: enemy_obj.kill()
        ...
        elif self.game_state == GAME_STATE_BONUS_LEVEL_PLAYING and not self.paused: self.update_bonus_level()
    ...
    def check_collisions(self):
        ...
        if not self.level_cleared_pending_animation:
            ...
            for ring_sprite in collided_rings_sprites:
                self.score += 10; self.play_sound('collect_ring'); self.collected_rings += 1; self.drone_system.add_player_cores(5)
                anim_ring_surf = None
                if hasattr(ring_sprite,'image'):
                    try: anim_ring_surf = pygame.transform.smoothscale(ring_sprite.image.copy(), (15,15))
                    except: anim_ring_surf = self._create_fallback_icon((15,15),"",GOLD)
        ...
        for item in collided_powerups:
            if not item.collected and not item.expired and hasattr(item,'apply_effect'):
                item.apply_effect(self.player); item.collected=True; item.kill(); self.play_sound('weapon_upgrade_collect'); self.score+=25
        ...
        if self.player: player_projectiles.add(self.player.bullets_group); player_projectiles.add(self.player.missiles_group)
        for projectile in list(player_projectiles):
            ...
            for enemy_obj in hit_enemies:
                if enemy_obj.alive:
                    enemy_obj.take_damage(projectile.damage); projectile.alive=False; projectile.kill()
                    if not enemy_obj.alive: self.score+=50; self.drone_system.add_player_cores(25)
        ...
        for enemy_obj in self.enemies:
            for bullet_obj in list(enemy_obj.bullets):
                if self.player.alive and bullet_obj.rect.colliderect(self.player.rect):
                    self.player.take_damage(ENEMY_BULLET_DAMAGE, self.sounds.get('crash')); bullet_obj.alive=False; bullet_obj.kill()
                if not bullet_obj.alive and bullet_obj in enemy_obj.bullets: enemy_obj.bullets.remove(bullet_obj)
    ...
    def draw_ui(self):
        ...
        current_time=pygame.time.get_ticks(); label_font=self.font_ui_text; value_font=self.font_ui_values
        emoji_general_font=self.font_ui_emoji_general; emoji_small_font=self.font_ui_emoji_small
        def render_text(text,font,color): return font.render(text,True,color)
        ...
        _health_icon_char_temp="❤️"; _health_icon_surf_temp=render_text(_health_icon_char_temp,emoji_small_font,RED)
        ...
        health_bar_y_pos=current_vitals_y-bar_height; health_icon_char="❤️"; health_icon_surf=render_text(health_icon_char,emoji_small_font,RED)
        ...
        weapon_bar_y_pos=current_vitals_y-bar_height; weapon_icon_char=WEAPON_MODE_ICONS.get(self.player.current_weapon_mode,"💥"); weapon_icon_surf=render_text(weapon_icon_char,emoji_small_font,ORANGE)
        ...
        if self.player.current_weapon_mode==get_game_setting("WEAPON_MODE_HEATSEEKER"): weapon_ready_color=MISSILE_COLOR; time_since_last_shot=current_time-self.player.last_missile_shot_time; cooldown_duration=self.player.current_missile_cooldown
        elif self.player.current_weapon_mode==get_game_setting("WEAPON_MODE_LIGHTNING"): weapon_ready_color=get_game_setting("LIGHTNING_COLOR")
        ...
        if active_powerup_for_ui and (self.player.shield_active or self.player.speed_boost_active):
            ...
            if powerup_icon_char:
                powerup_icon_surf=render_text(powerup_icon_char,emoji_small_font,WHITE)
        ...
        cores_emoji_char="💠"; cores_value_str=f" {self.drone_system.get_player_cores()}"; cores_icon_surf=render_text(cores_emoji_char,emoji_general_font,GOLD)
        cores_value_text_surf=render_text(cores_value_str,value_font,GOLD); cores_display_height=max(cores_icon_surf.get_height(),cores_value_text_surf.get_height())
        cores_y_pos=current_collectibles_y-cores_display_height; total_cores_width=cores_icon_surf.get_width()+text_icon_spacing+cores_value_text_surf.get_width()
        ...
        rings_y_pos = current_collectibles_y
        if self.ring_ui_icon:
        ...
        if TOTAL_CORE_FRAGMENTS_NEEDED>0:
            frag_text_str=f"Fragments: {num_fragments_collected}/{TOTAL_CORE_FRAGMENTS_NEEDED}"; frag_color=PURPLE if num_fragments_collected<TOTAL_CORE_FRAGMENTS_NEEDED else GOLD
            frag_surf=label_font.render(frag_text_str,True,frag_color)
        ...
        center_section_alloc_width=collectibles_approx_start_x-vitals_approx_end_x; center_section_mid_x=vitals_approx_end_x+(center_section_alloc_width/2)
        info_y_pos=panel_y_start+(panel_height-label_font.get_height())//2
        score_emoji_char="🏆 "; score_text_str=f"Score: {self.score}"; score_emoji_surf=render_text(score_emoji_char,emoji_general_font,GOLD); score_text_surf=render_text(score_text_str,label_font,GOLD)
        level_emoji_char="🎯 "; level_text_str = f"Level: {self.level}"
        if self.game_state == GAME_STATE_BONUS_LEVEL_PLAYING: level_text_str = "Bonus!"
        level_emoji_surf=render_text(level_emoji_char,emoji_general_font,CYAN); level_text_surf=render_text(level_text_str,label_font,CYAN)
        time_icon_char="⏱ "; time_ms_to_display=self.level_time_remaining_ms
        if self.game_state == GAME_STATE_BONUS_LEVEL_PLAYING: elapsed_bonus_time = pygame.time.get_ticks()-self.bonus_level_timer_start; time_ms_to_display = max(0,self.bonus_level_duration_ms-elapsed_bonus_time)
        time_value_str = f"{max(0,time_ms_to_display//1000)//60:02d}:{max(0,time_ms_to_display//1000)%60:02d}"; time_color=WHITE
        if(time_ms_to_display//1000)<=10: time_color=RED if(current_time//250)%2==0 else DARK_RED
        elif(time_ms_to_display//1000)<=30: time_color=YELLOW
        time_icon_surf=render_text(time_icon_char,emoji_general_font,time_color); time_value_surf=render_text(time_value_str,self.timer_display_font,time_color)
        ...
        self.screen.blit(score_emoji_surf,(current_info_x,info_y_pos+(score_text_surf.get_height()-score_emoji_surf.get_height())//2)); current_info_x+=score_emoji_surf.get_width()+text_icon_spacing
        self.screen.blit(score_text_surf,(current_info_x,info_y_pos)); current_info_x+=score_text_surf.get_width()+spacing_between_center_elements
        self.screen.blit(level_emoji_surf,(current_info_x,info_y_pos+(level_text_surf.get_height()-level_emoji_surf.get_height())//2)); current_info_x+=level_emoji_surf.get_width()+text_icon_spacing
        self.screen.blit(level_text_surf,(current_info_x,info_y_pos)); current_info_x+=level_text_surf.get_width()+spacing_between_center_elements
        self.screen.blit(time_icon_surf,(current_info_x,info_y_pos+(time_value_surf.get_height()-time_icon_surf.get_height())//2)); current_info_x+=time_icon_surf.get_width()+text_icon_spacing
        ...
        if self.total_rings_per_level > 0 and self.ring_ui_icon:
            ...
            _target_ring_row_y_for_anim = rings_y_pos
        ...
        for ring_anim in self.animating_rings:
            if 'surface' in ring_anim and ring_anim['surface']: self.screen.blit(ring_anim['surface'], (int(ring_anim['pos'][0]),int(ring_anim['pos'][1])))
    ...
    def draw_overlay(self):
        if self.game_state == GAME_STATE_GAME_OVER:
            ...
            go_text = self.large_font.render("GAME OVER", True, RED); sc_text = self.font.render(f"Final Score: {self.score}", True, WHITE)
            self.screen.blit(go_text, go_text.get_rect(center=(WIDTH//2, HEIGHT//2-100))); self.screen.blit(sc_text, sc_text.get_rect(center=(WIDTH//2, HEIGHT//2-20)))
            can_submit_score=not get_game_setting("SETTINGS_MODIFIED"); is_new_high=can_submit_score and leaderboard.is_high_score(self.score,self.level); prompt_y_offset=HEIGHT//2+40
            if not can_submit_score: no_lb_text=self.font.render("Leaderboard disabled (custom settings).",True,YELLOW); self.screen.blit(no_lb_text,no_lb_text.get_rect(center=(WIDTH//2,prompt_y_offset))); prompt_y_offset+=self.font.get_height()+10
            ...
            prompt_surf = self.font.render(prompt_str,True,prompt_clr); self.screen.blit(prompt_surf,prompt_surf.get_rect(center=(WIDTH//2,prompt_y_offset)))
        elif self.game_state == GAME_STATE_ENTER_NAME:
            overlay = pygame.Surface((WIDTH,HEIGHT),pygame.SRCALPHA); overlay.fill((0,0,0,200)); self.screen.blit(overlay,(0,0))
            self.screen.blit(self.large_font.render("New High Score!",True,GOLD),self.large_font.render("New High Score!",True,GOLD).get_rect(center=(WIDTH//2,HEIGHT//2-150)))
            self.screen.blit(self.font.render(f"Your Score: {self.score} (Level: {self.level})",True,WHITE),self.font.render(f"Your Score: {self.score} (Level: {self.level})",True,WHITE).get_rect(center=(WIDTH//2,HEIGHT//2-80)))
            self.screen.blit(self.font.render("Enter Name (6 chars, A-Z):",True,WHITE),self.font.render("Enter Name (6 chars, A-Z):",True,WHITE).get_rect(center=(WIDTH//2,HEIGHT//2-20)))
            input_box = pygame.Rect(WIDTH//2-125,HEIGHT//2+30,250,50); pygame.draw.rect(self.screen,WHITE,input_box,2)
            self.screen.blit(self.input_font.render(self.player_name_input,True,WHITE),self.input_font.render(self.player_name_input,True,WHITE).get_rect(center=input_box.center))
            self.screen.blit(self.font.render("Press ENTER to submit.",True,CYAN),self.font.render("Press ENTER to submit.",True,CYAN).get_rect(center=(WIDTH//2,HEIGHT//2+120)))
        elif self.game_state == GAME_STATE_LEADERBOARD:
            self.screen.fill(BLACK)
            if self.menu_stars:
                for star_params in self.menu_stars: pygame.draw.circle(self.screen,WHITE,(int(star_params[0]),int(star_params[1])),star_params[3])
            ...
            title_bg=pygame.Surface((title_surf.get_width()+30,title_surf.get_height()+15),pygame.SRCALPHA); title_bg.fill((20,20,20,180))
            title_bg.blit(title_surf,title_surf.get_rect(center=(title_bg.get_width()//2,title_bg.get_height()//2))); self.screen.blit(title_bg,title_bg.get_rect(center=(WIDTH//2,HEIGHT//2-300)))
            scores_to_display = leaderboard.get_top_scores(); score_item_y_start = HEIGHT//2-200; header_y = HEIGHT//2-250; item_line_height = self.font_default_text.get_height()+10
            if not scores_to_display:
                no_scores_surf=self.font_default_text.render("No scores yet!",True,WHITE); no_scores_bg=pygame.Surface((no_scores_surf.get_width()+20,no_scores_surf.get_height()+10),pygame.SRCALPHA)
                no_scores_bg.fill((30,30,30,160)); no_scores_bg.blit(no_scores_surf,(10,5)); self.screen.blit(no_scores_bg,no_scores_bg.get_rect(center=(WIDTH//2,HEIGHT//2)))
            else:
                cols = {"Rank":WIDTH//2-350,"Name":WIDTH//2-250,"Level":WIDTH//2+50,"Score":WIDTH//2+200}
                for col_name,x_pos in cols.items():
                    header_surf=self.font_default_text.render(col_name,True,WHITE); header_bg=pygame.Surface((header_surf.get_width()+10,header_surf.get_height()+5),pygame.SRCALPHA)
                    header_bg.fill((40,40,40,170)); header_bg.blit(header_surf,(5,2)); self.screen.blit(header_bg,(x_pos,header_y))
                for i,entry in enumerate(scores_to_display):
                    y_pos = score_item_y_start + i * item_line_height
                    texts_to_draw = [(f"{i+1}.",WHITE,cols["Rank"]),(entry.get('name','N/A'),CYAN,cols["Name"]),(str(entry.get('level','-')),GREEN,cols["Level"]),(str(entry.get('score',0)),GOLD,cols["Score"])]
                    for text_str,color,x_coord in texts_to_draw:
                        text_surf=self.font_default_text.render(text_str,True,color); item_bg=pygame.Surface((text_surf.get_width()+8,text_surf.get_height()+4),pygame.SRCALPHA)
                        item_bg.fill((30,30,30,150)); item_bg.blit(text_surf,(4,2)); self.screen.blit(item_bg,(x_coord,y_pos))
            menu_prompt_surf=self.font_default_text.render("ESC: Main Menu | Q: Quit",True,WHITE)
        ...
        elif (self.game_state == GAME_STATE_PLAYING or self.game_state == GAME_STATE_BONUS_LEVEL_PLAYING) and self.paused:
            overlay = pygame.Surface((WIDTH,HEIGHT),pygame.SRCALPHA); overlay.fill((0,0,0,128)); self.screen.blit(overlay,(0,0))
            self.screen.blit(self.large_font.render("PAUSED",True,WHITE),self.large_font.render("PAUSED",True,WHITE).get_rect(center=(WIDTH//2,HEIGHT//2-40)))
            pause_text = "P: Continue, L: Leaderboard, M: Menu, Q: Quit"
            if self.game_state == GAME_STATE_BONUS_LEVEL_PLAYING: pause_text = "P: Continue, ESC: End Bonus, M: Menu, Q: Quit"
            self.screen.blit(self.font.render(pause_text,True,WHITE),self.font.render(pause_text,True,WHITE).get_rect(center=(WIDTH//2,HEIGHT//2+40)))

    def draw_bonus_level_placeholder(self):
        self.screen.fill(DARK_RED)
        if self.maze: self.maze.draw(self.screen)
        if self.player: self.player.draw(self.screen)
        title_surf = self.large_font.render("== BONUS LEVEL ==", True, GOLD)
        self.screen.blit(title_surf, title_surf.get_rect(center=(WIDTH//2, HEIGHT//4)))
        objective = "Survive!"
        if self.current_bonus_level_objective == "collect_cores": objective = "Collect Max Cores!"
        obj_text = f"Objective: {objective}"
        obj_surf = self.medium_font.render(obj_text, True, WHITE)
        self.screen.blit(obj_surf, obj_surf.get_rect(center=(WIDTH//2, HEIGHT//2)))
        self.draw_ui()
    ...
    def draw(self):
        ...
        if self.game_state == GAME_STATE_MAIN_MENU: self.draw_main_menu()
        ...
        elif self.game_state == GAME_STATE_BONUS_LEVEL_PLAYING:
            self.draw_bonus_level_placeholder()
            ...
            self.draw_ui()
            if self.game_state != GAME_STATE_PLAYING or self.paused: self.draw_overlay()
        pygame.display.flip()
