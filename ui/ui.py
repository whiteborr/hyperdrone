...
try:
    ...
    print("UIManager: Could not import BuildMenu. Build UI will not be available.")
    BuildMenu = None 
...
class UIManager:
    ...
    def _load_ui_assets(self):
        ...
        if CORE_FRAGMENT_DETAILS:
            for frag_key, details in CORE_FRAGMENT_DETAILS.items():
                frag_id = details.get("id"); icon_filename = details.get("icon_filename")
    ...
    def update_player_life_icon_surface(self):
        ...
        if icon_path and os.path.exists(icon_path):
            try:
                ...
                print(f"UIManager: Error loading life icon '{icon_path}': {e}")
    ...
    def draw_current_scene_ui(self):
        ...
        ui_flow_ctrl = self.game_controller.ui_flow_controller # Access UIFlowController
        ...
        # Routing to specific draw methods
    ...
    def draw_game_intro_scroll(self):
        ...
        if current_image_surface: 
            img_w, img_h = current_image_surface.get_size(); aspect_ratio = img_h / img_w if img_w > 0 else 1
            scaled_w = WIDTH; scaled_h = int(scaled_w * aspect_ratio)
            pos_y = (HEIGHT - scaled_h) // 2; pos_x = 0
            if scaled_h > HEIGHT: scaled_h = HEIGHT; scaled_w = int(scaled_h / aspect_ratio if aspect_ratio > 0 else WIDTH); pos_x = (WIDTH - scaled_w) // 2; pos_y = 0
            try: self.screen.blit(pygame.transform.smoothscale(current_image_surface, (scaled_w, scaled_h)), (pos_x, pos_y))
            except pygame.error as e:
                print(f"UIManager: Error scaling/blitting intro image: {e}")
                if ui_flow_ctrl and ui_flow_ctrl.menu_stars:
                    for star_params in ui_flow_ctrl.menu_stars: pygame.draw.circle(self.screen, WHITE, (int(star_params[0]), int(star_params[1])), star_params[3])
        else:
            if ui_flow_ctrl and ui_flow_ctrl.menu_stars:
                for star_params in ui_flow_ctrl.menu_stars: pygame.draw.circle(self.screen, WHITE, (int(star_params[0]), int(star_params[1])), star_params[3])
    ...
    def draw_story_message_overlay(self, message):
        ...
        line_height = font.get_linesize(); effective_line_spacing = line_height * line_spacing_ratio
        ...
        box_width = max_line_width + 2 * padding; box_height = total_text_height + 2 * padding
        box_x = (WIDTH - box_width) // 2; box_y = GAME_PLAY_AREA_HEIGHT - box_height - 20 
        overlay_surf = pygame.Surface((box_width, box_height), pygame.SRCALPHA)
        overlay_surf.fill((10, 20, 40, 220)); pygame.draw.rect(overlay_surf, GOLD, overlay_surf.get_rect(), 2, border_radius=10)
    ...
    def draw_codex_screen(self):
        ui_flow_ctrl = self.game_controller.ui_flow_controller
        # (Logic remains the same, uses ui_flow_ctrl for codex state variables)
        self.screen.fill(BLACK)
        if ui_flow_ctrl.menu_stars: 
            for star_params in ui_flow_ctrl.menu_stars: pygame.draw.circle(self.screen, (50,50,50), (int(star_params[0]), int(star_params[1])), star_params[3])
        title_surf = self._render_text_safe("Lore Codex", "codex_title_font", GOLD)
        title_rect = title_surf.get_rect(center=(WIDTH // 2, 60)); self.screen.blit(title_surf, title_rect)
        current_view = ui_flow_ctrl.codex_current_view
        padding = 50; list_panel_width = WIDTH // 3 - padding * 1.5; list_panel_x = padding
        content_panel_x = list_panel_x + list_panel_width + padding / 2
        content_panel_width = WIDTH - content_panel_x - padding
        top_y_start = title_rect.bottom + 30; bottom_y_end = HEIGHT - 80
        ...
        if not all([category_font, entry_font, content_font]):
            ...
            self.screen.blit(fallback_surf, fallback_surf.get_rect(center=(WIDTH // 2, HEIGHT // 2))); return
        if self.codex_list_item_height == 0:
             self.codex_list_item_height = entry_font.get_height() + 15
        ...
        if current_view == "categories":
            categories = ui_flow_ctrl.codex_categories_list; selected_category_idx = ui_flow_ctrl.codex_selected_category_index
            if not categories: self.screen.blit(self._render_text_safe("No lore unlocked.", "medium_text", WHITE), self._render_text_safe("No lore unlocked.", "medium_text", WHITE).get_rect(center=(WIDTH // 2, HEIGHT // 2)))
            else:
            ...
            category_name = ui_flow_ctrl.codex_current_category_name; entries = ui_flow_ctrl.codex_entries_in_category_list; selected_entry_idx = ui_flow_ctrl.codex_selected_entry_index_in_category
            cat_title_surf = self._render_text_safe(f"{category_name}", "codex_category_font", GOLD); self.screen.blit(cat_title_surf, (list_panel_x + 10, top_y_start))
            current_list_y = top_y_start + cat_title_surf.get_height() + 15
            if not entries: self.screen.blit(self._render_text_safe("No entries here.", "codex_entry_font", GREY), (list_panel_x + 20, current_list_y))
            else:
                max_visible = self.codex_max_visible_items_list if self.codex_max_visible_items_list > 0 else 1
                ...
                for i_display, i_actual in enumerate(range(start_idx, end_idx)):
                    entry_data = entries[i_actual]; y_pos = current_list_y + i_display * self.codex_list_item_height; color = YELLOW if i_actual == selected_entry_idx else WHITE
            ...
            selected_entry_id = ui_flow_ctrl.codex_selected_entry_id; entry_data = self.drone_system.get_lore_entry_details(selected_entry_id) if selected_entry_id else None
            category_name_reminder = ui_flow_ctrl.codex_current_category_name; is_drone_entry = entry_data.get("category") == "Drones" if entry_data else False; is_race_entry = entry_data.get("category") == "Alien Races" if entry_data else False
            image_path = entry_data.get("image") if entry_data else None; current_image_y_pos = top_y_start + 20
            ...
            if entry_data:
                content_title_surf = self._render_text_safe(entry_data.get("title", "Untitled"), "codex_category_font", GOLD); self.screen.blit(content_title_surf, (content_panel_x, top_y_start))
                content_text_render_y = top_y_start + content_title_surf.get_height() + 20; text_area_width = content_panel_width - 20
                if is_drone_entry and image_path: # Drone image display logic
                    if image_path not in self.codex_image_cache:
                        ...
                        except pygame.error as e: self.codex_image_cache[image_path] = None
                    cached_image = self.codex_image_cache.get(image_path)
                    if cached_image:
                        img_max_w = list_panel_width - 20; img_max_h = HEIGHT * 0.3; original_w, original_h = cached_image.get_size(); aspect = original_h / original_w if original_w > 0 else 1
                        scaled_w = img_max_w; scaled_h = int(scaled_w * aspect)
                        if scaled_h > img_max_h: scaled_h = int(img_max_h); scaled_w = int(scaled_h / aspect if aspect > 0 else img_max_w)
                        try: self.screen.blit(pygame.transform.smoothscale(cached_image, (scaled_w, scaled_h)), (list_panel_x + (list_panel_width - scaled_w) // 2, current_image_y_pos))
                        except pygame.error as e: print(f"UIManager: Error scaling Drone Codex image '{image_path}': {e}")
                content_text = entry_data.get("content", "No content available."); scroll_offset_lines = ui_flow_ctrl.codex_content_scroll_offset
                wrapped_lines = self._wrap_text(content_text, content_font, text_area_width); ui_flow_ctrl.codex_current_entry_total_lines = len(wrapped_lines)
                ...
                if is_race_entry and image_path: # Race image display logic
                    if image_path not in self.codex_image_cache:
                        try: self.codex_image_cache[image_path] = pygame.image.load(image_path).convert_alpha()
                        except pygame.error as e: self.codex_image_cache[image_path] = None
                    cached_race_image = self.codex_image_cache.get(image_path)
                    if cached_race_image:
                        img_max_w_race = content_panel_width * 0.6; img_max_h_race = HEIGHT * 0.25; original_w_race, original_h_race = cached_race_image.get_size(); aspect_race = original_h_race / original_w_race if original_w_race > 0 else 1
                        ...
                        if scaled_w_race > 0 and scaled_h_race > 0:
                            try: race_image_to_draw_below_text = pygame.transform.smoothscale(cached_race_image, (scaled_w_race, scaled_h_race)); scaled_race_img_h = scaled_h_race; text_content_area_available_height -= (scaled_race_img_h + 20)
                ...
                if race_image_to_draw_below_text:
                    ...
                    if race_img_y_pos + scaled_race_img_h < bottom_y_end: self.screen.blit(race_image_to_draw_below_text, (content_panel_x + (text_area_width - race_image_to_draw_below_text.get_width()) // 2, race_img_y_pos))
                if len(wrapped_lines) > max_lines: # Scroll indicators
                    if scroll_offset_lines > 0: self.screen.blit(self._render_text_safe("▲ Up", "small_text", YELLOW), (content_panel_x + text_area_width - self._render_text_safe("▲ Up", "small_text", YELLOW).get_width(), content_text_render_y - 25))
                ...
                self.screen.blit(self._render_text_safe("Error: Could not load entry content.", "medium_text", RED), self._render_text_safe("Error: Could not load entry content.", "medium_text", RED).get_rect(center=(content_panel_x + content_panel_width // 2, HEIGHT // 2))); nav_instr = "ESC: Back"
    ...
    def draw_main_menu(self):
        ui_flow_ctrl = self.game_controller.ui_flow_controller
        # (Logic remains the same, using ui_flow_ctrl for menu_options, selected_menu_option, menu_stars)
        # (font_path_neuropol from self.game_controller)
        if self.ui_assets["menu_background"]:
            try: self.screen.blit(pygame.transform.smoothscale(self.ui_assets["menu_background"], (WIDTH, HEIGHT)), (0,0))
            except Exception as e: print(f"UIManager: Error blitting menu background: {e}"); self.screen.fill(BLACK) 
        else: self.screen.fill(BLACK) 
        if ui_flow_ctrl.menu_stars:
            for star_params in ui_flow_ctrl.menu_stars: pygame.draw.circle(self.screen, WHITE, (int(star_params[0]), int(star_params[1])), star_params[3])
        selected_option_idx = ui_flow_ctrl.selected_menu_option; menu_item_start_y = HEIGHT // 2 - 80 ; item_spacing = 85 
        unselected_label_font_size = 48; selected_label_font_size = 54; temp_icon_font = self.fonts.get("ui_emoji_general", self.fonts.get("menu_text"))
        font_path_neuropol = self.game_controller.font_path_neuropol
        menu_options_with_icons = { "Start Game": "🛸 Start Game", "Maze Defense": "🛡️ Maze Defense", "Select Drone": "\U0001F579 Select Drone", "Codex": "📜 Codex", "Settings": "\u2699 Settings", "Leaderboard": "🏆 Leaderboard", "Quit": "❌ Quit" }
        ...
        for option_key_calc in actual_menu_options:
            full_label_calc = menu_options_with_icons.get(option_key_calc, option_key_calc); icon_char_calc = ""; label_text_calc = full_label_calc
            if len(full_label_calc) >= 2 and full_label_calc[1] == " " and not full_label_calc[0].isalnum(): icon_char_calc = full_label_calc[0]; label_text_calc = full_label_calc[2:]
            try: temp_label_font_for_calc = pygame.font.Font(font_path_neuropol, selected_label_font_size) if font_path_neuropol else pygame.font.Font(None, selected_label_font_size)
            except Exception: temp_label_font_for_calc = pygame.font.Font(None, selected_label_font_size)
            ...
            if icon_surf_calc: current_combined_width += icon_surf_calc.get_width() + icon_label_spacing; current_content_height = max(current_content_height, icon_surf_calc.get_height())
            current_combined_width += label_surf_calc.get_width(); current_content_height = max(current_content_height, label_surf_calc.get_height())
            max_combined_width = max(max_combined_width, current_combined_width); max_content_height = max(max_content_height, current_content_height)
        horizontal_padding = 40 ; vertical_padding = 20; min_button_width = 300 ; fixed_button_width = max(min_button_width, max_combined_width + horizontal_padding); fixed_button_height = max_content_height + vertical_padding
        for i, option_key in enumerate(actual_menu_options):
            full_label = menu_options_with_icons.get(option_key, option_key); is_selected = (i == selected_option_idx); text_color = GOLD if is_selected else WHITE
            icon_char = ""; label_text = full_label
            if len(full_label) >= 2 and full_label[1] == " " and not full_label[0].isalnum(): icon_char = full_label[0]; label_text = full_label[2:]
            current_label_font_size = selected_label_font_size if is_selected else unselected_label_font_size
            try: label_font = pygame.font.Font(font_path_neuropol, current_label_font_size) if font_path_neuropol else pygame.font.Font(None, current_label_font_size)
            except Exception: label_font = pygame.font.Font(None, current_label_font_size)
            icon_font = temp_icon_font; icon_surf = icon_font.render(icon_char, True, text_color) if icon_char else None
            ...
            bg_color = (70, 70, 70, 220) if is_selected else (50, 50, 50, 180); pygame.draw.rect(button_surface, bg_color, button_surface.get_rect(), border_radius=15)
            ...
            if icon_surf: icon_rect = icon_surf.get_rect(centery=y_center_content); icon_rect.left = content_start_x; button_surface.blit(icon_surf, icon_rect); content_start_x = icon_rect.right + icon_label_spacing
            label_rect = label_surf.get_rect(centery=y_center_content); label_rect.left = content_start_x; button_surface.blit(label_surf, label_rect)
            current_button_y = menu_item_start_y + i * item_spacing; button_screen_rect = button_surface.get_rect(center=(WIDTH // 2, current_button_y))
            self.screen.blit(button_surface, button_screen_rect)
        instr_text = "Use UP/DOWN keys, ENTER to select."; instr_surf = self._render_text_safe(instr_text, "small_text", self.INSTRUCTION_TEXT_COLOR)
        ...
        instr_bg_box.fill(self.INSTRUCTION_BG_COLOR); instr_bg_box.blit(instr_surf, instr_surf.get_rect(center=(instr_bg_box.get_width() // 2, instr_bg_box.get_height() // 2)))
        ...
        if gs.SETTINGS_MODIFIED:
            ...
            warning_bg_box.fill(self.INSTRUCTION_BG_COLOR) ; warning_bg_box.blit(warning_surf, warning_surf.get_rect(center=(warning_bg_box.get_width() // 2, warning_bg_box.get_height() // 2)))
    ...
    def draw_drone_select_menu(self):
        ui_flow_ctrl = self.game_controller.ui_flow_controller
        # (Logic remains the same, using ui_flow_ctrl for drone_select_options, selected_drone_preview_index)
        # (drone_config, drone_stats from self.drone_system)
        # (drone_image_surf from self.game_controller.drone_main_display_cache)
        title_surf = self._render_text_safe("Select Drone", "title_text", GOLD); title_rect = title_surf.get_rect(center=(WIDTH // 2, 70)); self.screen.blit(title_surf, title_rect)
        drone_options_ids = ui_flow_ctrl.drone_select_options; selected_preview_idx = ui_flow_ctrl.selected_drone_preview_index
        if not drone_options_ids: self.screen.blit(self._render_text_safe("NO DRONES AVAILABLE", "large_text", RED), self._render_text_safe("NO DRONES AVAILABLE", "large_text", RED).get_rect(center=(WIDTH // 2, HEIGHT // 2))); return
        current_drone_id = drone_options_ids[selected_preview_idx]; drone_config = self.drone_system.get_drone_config(current_drone_id)
        drone_stats = self.drone_system.get_drone_stats(current_drone_id, is_in_architect_vault=False); is_unlocked = self.drone_system.is_drone_unlocked(current_drone_id)
        is_currently_equipped = (current_drone_id == self.drone_system.get_selected_drone_id()); drone_image_surf = self.game_controller.drone_main_display_cache.get(current_drone_id)
        img_width = drone_image_surf.get_width() if drone_image_surf else 200; img_height = drone_image_surf.get_height() if drone_image_surf else 200
        name_text = drone_config.get("name", "N/A"); name_surf_temp = self.fonts["drone_name_cycle"].render(name_text, True, WHITE); name_height = name_surf_temp.get_height()
        hp_stat = drone_stats.get("hp"); speed_stat = drone_stats.get("speed"); turn_speed_stat = drone_stats.get("turn_speed"); fire_rate_mult = drone_stats.get("fire_rate_multiplier", 1.0); special_ability_key = drone_stats.get("special_ability")
        hp_display = str(hp_stat) if hp_stat is not None else "N/A"; speed_display = f"{speed_stat:.1f}" if isinstance(speed_stat, (int, float)) else "N/A"; turn_speed_display = f"{turn_speed_stat:.1f}" if isinstance(turn_speed_stat, (int, float)) else "N/A"
        ...
        stats_content_surfaces = []; max_stat_label_w = 0; max_stat_value_w = 0; stat_line_h = self.fonts["drone_stats_label_cycle"].get_height() + 5
        for label_str, value_str in stats_data_tuples:
            label_s = self._render_text_safe(label_str, "drone_stats_label_cycle", LIGHT_BLUE if is_unlocked else GREY); value_s = self._render_text_safe(value_str, "drone_stats_value_cycle", WHITE if is_unlocked else GREY)
            stats_content_surfaces.append((label_s, value_s)); max_stat_label_w = max(max_stat_label_w, label_s.get_width()); max_stat_value_w = max(max_stat_value_w, value_s.get_width())
        stats_box_padding = 15; stats_box_visual_width = max_stat_label_w + max_stat_value_w + 3 * stats_box_padding; stats_box_visual_height = (len(stats_content_surfaces) * stat_line_h) - (5 if stats_content_surfaces else 0) + 2 * stats_box_padding
        desc_text = drone_config.get("description", ""); desc_color_final = (200,200,200) if is_unlocked else (100,100,100); desc_max_width_for_card = WIDTH * 0.45; desc_lines_surfs = []
        words = desc_text.split(' '); current_line_text_desc = ""; desc_font = self.fonts["drone_desc_cycle"]
        for word in words:
            test_line = current_line_text_desc + word + " ";
            if desc_font.size(test_line)[0] < desc_max_width_for_card: current_line_text_desc = test_line
            else: desc_lines_surfs.append(self._render_text_safe(current_line_text_desc.strip(), "drone_desc_cycle", desc_color_final)); current_line_text_desc = word + " "
        if current_line_text_desc: desc_lines_surfs.append(self._render_text_safe(current_line_text_desc.strip(), "drone_desc_cycle", desc_color_final))
        total_desc_height = sum(s.get_height() for s in desc_lines_surfs) + (len(desc_lines_surfs)-1)*3 if desc_lines_surfs else 0
        unlock_text_str = ""; unlock_text_color = WHITE; unlock_condition = drone_config.get("unlock_condition", {})
        if not is_unlocked:
            condition_text_str = unlock_condition.get("description", "Locked"); unlock_cost_val = unlock_condition.get("value"); type_is_cores_unlock = unlock_condition.get("type") == "cores"
            unlock_text_str = condition_text_str
            if type_is_cores_unlock and unlock_cost_val is not None:
                 can_afford = self.drone_system.get_player_cores() >= unlock_cost_val; unlock_text_str = f"Unlock: {unlock_cost_val} 💠 "; unlock_text_str += "(ENTER)" if can_afford else "(Not Enough Cores)"; unlock_text_color = GREEN if can_afford else YELLOW
        ...
        unlock_info_surf = self._render_text_safe(unlock_text_str, "drone_unlock_cycle", unlock_text_color); unlock_info_height = unlock_info_surf.get_height() if unlock_info_surf else 0
        spacing_between_elements = 15; padding_inside_card = 25
        card_content_total_h = (img_height + spacing_between_elements + name_height + spacing_between_elements + stats_box_visual_height + spacing_between_elements + total_desc_height + spacing_between_elements + unlock_info_height)
        max_content_width_for_card = max(img_width, name_surf_temp.get_width(), stats_box_visual_width, max(s.get_width() for s in desc_lines_surfs) if desc_lines_surfs else 0, unlock_info_surf.get_width() if unlock_info_surf else 0)
        card_w = max_content_width_for_card + 2 * padding_inside_card; card_w = min(card_w, WIDTH * 0.6); card_h = card_content_total_h + 2 * padding_inside_card + 20
        title_bottom = title_rect.bottom if title_rect else 100; main_card_x = (WIDTH - card_w) // 2; main_card_y = title_bottom + 40
        main_card_rect = pygame.Rect(main_card_x, main_card_y, card_w, card_h)
        pygame.draw.rect(self.screen, (25,30,40,230), main_card_rect, border_radius=20); pygame.draw.rect(self.screen, GOLD, main_card_rect, 3, border_radius=20)
        current_y_in_card = main_card_rect.top + padding_inside_card
        if drone_image_surf:
            display_drone_image = drone_image_surf
            if not is_unlocked: temp_img = drone_image_surf.copy(); temp_img.set_alpha(100); display_drone_image = temp_img
            final_img_rect = display_drone_image.get_rect(centerx=main_card_rect.centerx, top=current_y_in_card); self.screen.blit(display_drone_image, final_img_rect); current_y_in_card = final_img_rect.bottom + spacing_between_elements
        else: current_y_in_card += img_height + spacing_between_elements
        name_color_final = WHITE if is_unlocked else GREY; name_surf_final = self._render_text_safe(name_text, "drone_name_cycle", name_color_final)
        final_name_rect = name_surf_final.get_rect(centerx=main_card_rect.centerx, top=current_y_in_card); self.screen.blit(name_surf_final, final_name_rect); current_y_in_card = final_name_rect.bottom + spacing_between_elements
        ...
        pygame.draw.rect(self.screen, (40,45,55,200), final_stats_box_draw_rect, border_radius=10); pygame.draw.rect(self.screen, CYAN, final_stats_box_draw_rect, 1, border_radius=10)
        ...
        for i, (label_s, value_s) in enumerate(stats_content_surfaces):
            self.screen.blit(label_s, (final_stats_box_draw_rect.left + stats_box_padding, stat_y_pos_render)); self.screen.blit(value_s, (final_stats_box_draw_rect.right - stats_box_padding - value_s.get_width(), stat_y_pos_render))
            stat_y_pos_render += max(label_s.get_height(), value_s.get_height()) + (5 if i < len(stats_content_surfaces)-1 else 0)
        current_y_in_card = final_stats_box_draw_rect.bottom + spacing_between_elements; desc_start_y_render = current_y_in_card
        for line_surf in desc_lines_surfs: self.screen.blit(line_surf, line_surf.get_rect(centerx=main_card_rect.centerx, top=desc_start_y_render)); desc_start_y_render += line_surf.get_height() + 3
        current_y_in_card = desc_start_y_render + 5
        if unlock_info_surf: unlock_info_rect = unlock_info_surf.get_rect(centerx=main_card_rect.centerx, top=current_y_in_card); self.screen.blit(unlock_info_surf, unlock_info_rect)
        arrow_font = self.fonts.get("arrow_font_key", self.fonts["large_text"]); left_arrow_surf = arrow_font.render("◀", True, WHITE if len(drone_options_ids) > 1 else GREY); right_arrow_surf = arrow_font.render("▶", True, WHITE if len(drone_options_ids) > 1 else GREY)
        arrow_y_center = main_card_rect.centery; arrow_padding_from_card_edge = 40
        if len(drone_options_ids) > 1:
            left_arrow_rect = left_arrow_surf.get_rect(centery=arrow_y_center, right=main_card_rect.left - arrow_padding_from_card_edge); self.screen.blit(left_arrow_surf, left_arrow_rect)
            right_arrow_rect = right_arrow_surf.get_rect(centery=arrow_y_center, left=main_card_rect.right + arrow_padding_from_card_edge); self.screen.blit(right_arrow_surf, right_arrow_rect)
        instr_text = "LEFT/RIGHT: Cycle | ENTER: Select/Unlock | ESC: Back"; instr_surf = self._render_text_safe(instr_text, "small_text", self.INSTRUCTION_TEXT_COLOR)
        instr_bg_box = pygame.Surface((instr_surf.get_width() + self.INSTRUCTION_PADDING_X, instr_surf.get_height() + self.INSTRUCTION_PADDING_Y), pygame.SRCALPHA)
        instr_bg_box.fill(self.INSTRUCTION_BG_COLOR); instr_bg_box.blit(instr_surf, instr_surf.get_rect(center=(instr_bg_box.get_width() // 2, instr_bg_box.get_height() // 2)))
        self.screen.blit(instr_bg_box, instr_bg_box.get_rect(center=(WIDTH // 2, self.BOTTOM_INSTRUCTION_CENTER_Y)))
        cores_label_text_surf = self._render_text_safe(f"Player Cores: ", "ui_text", GOLD); cores_value_text_surf = self._render_text_safe(f"{self.drone_system.get_player_cores()}", "ui_values", GOLD); cores_emoji_surf = self._render_text_safe(" 💠", "ui_emoji_general", GOLD)
        total_cores_display_width = cores_label_text_surf.get_width() + cores_value_text_surf.get_width() + cores_emoji_surf.get_width(); cores_start_x = WIDTH - 20 - total_cores_display_width
        ...
        cores_y_baseline = self.BOTTOM_INSTRUCTION_CENTER_Y - (instr_bg_box.get_height() // 2) - 10 - max_element_height_cores ; current_x_offset_cores = cores_start_x
        self.screen.blit(cores_label_text_surf, (current_x_offset_cores, cores_y_baseline + (max_element_height_cores - cores_label_text_surf.get_height()) // 2)); current_x_offset_cores += cores_label_text_surf.get_width()
        self.screen.blit(cores_value_text_surf, (current_x_offset_cores, cores_y_baseline + (max_element_height_cores - cores_value_text_surf.get_height()) // 2)); current_x_offset_cores += cores_value_text_surf.get_width()
    ...
    def draw_settings_menu(self):
        ui_flow_ctrl = self.game_controller.ui_flow_controller
        # (Logic remains the same, using ui_flow_ctrl for settings_items_data, selected_setting_index)
        title_surf = self._render_text_safe("Settings", "title_text", GOLD); title_bg = pygame.Surface((title_surf.get_width()+30, title_surf.get_height()+15), pygame.SRCALPHA)
        title_bg.fill((20,20,20,180)); title_bg.blit(title_surf, title_surf.get_rect(center=(title_bg.get_width()//2, title_bg.get_height()//2))); self.screen.blit(title_bg, title_bg.get_rect(center=(WIDTH//2, 80)))
        settings_items = ui_flow_ctrl.settings_items_data; selected_idx = ui_flow_ctrl.selected_setting_index
        item_y_start = 180; item_line_height = self.fonts["ui_text"].get_height() + 20; max_items_on_screen = (HEIGHT - item_y_start - 120) // item_line_height 
        view_start_index = 0
        if len(settings_items) > max_items_on_screen: view_start_index = max(0, selected_idx - max_items_on_screen // 2); view_start_index = min(view_start_index, len(settings_items) - max_items_on_screen)
        ...
        for i_display, list_idx in enumerate(range(view_start_index, view_end_index)):
            ...
            item = settings_items[list_idx]; y_pos = item_y_start + i_display * item_line_height; color = YELLOW if list_idx == selected_idx else WHITE
            label_surf = self._render_text_safe(item["label"], "ui_text", color); label_bg_rect_width = max(250, label_surf.get_width() + 20)
            label_bg_rect = pygame.Rect(WIDTH // 4 - 150, y_pos - 5, label_bg_rect_width, label_surf.get_height() + 10)
            pygame.draw.rect(self.screen, (30,30,30,160), label_bg_rect, border_radius=5); self.screen.blit(label_surf, (label_bg_rect.left + 10, y_pos))
            ...
            if item["type"] != "action":
                ...
                value_surf = self._render_text_safe(display_value, "ui_text", color); value_bg_rect_width = max(100, value_surf.get_width() + 20)
                value_bg_rect = pygame.Rect(WIDTH // 2 + 150, y_pos - 5, value_bg_rect_width, value_surf.get_height() + 10)
                pygame.draw.rect(self.screen, (30,30,30,160), value_bg_rect, border_radius=5); self.screen.blit(value_surf, (value_bg_rect.left + 10, y_pos))
                if item["key"] in DEFAULT_SETTINGS and current_value != DEFAULT_SETTINGS[item["key"]]: self.screen.blit(self._render_text_safe("*", "small_text", RED), (value_bg_rect.right + 5, y_pos))
            elif list_idx == selected_idx:
                 action_hint_surf = self._render_text_safe("<ENTER>", "ui_text", YELLOW); action_hint_bg_rect = pygame.Rect(WIDTH // 2 + 150, y_pos - 5, action_hint_surf.get_width() + 20, action_hint_surf.get_height() + 10)
                 pygame.draw.rect(self.screen, (40,40,40,180), action_hint_bg_rect, border_radius=5); self.screen.blit(action_hint_surf, (action_hint_bg_rect.left + 10, y_pos))
        instr_text = "UP/DOWN: Select | LEFT/RIGHT: Adjust | ENTER: Activate | ESC: Back"; instr_surf = self._render_text_safe(instr_text, "small_text", self.INSTRUCTION_TEXT_COLOR)
        instr_bg_box = pygame.Surface((instr_surf.get_width() + self.INSTRUCTION_PADDING_X, instr_surf.get_height() + self.INSTRUCTION_PADDING_Y), pygame.SRCALPHA)
        instr_bg_box.fill(self.INSTRUCTION_BG_COLOR); instr_bg_box.blit(instr_surf, instr_surf.get_rect(center=(instr_bg_box.get_width() // 2, instr_bg_box.get_height() // 2)))
        self.screen.blit(instr_bg_box, instr_bg_box.get_rect(center=(WIDTH // 2, self.BOTTOM_INSTRUCTION_CENTER_Y)))
        if gs.SETTINGS_MODIFIED:
            warning_text = "Leaderboard disabled: settings changed from default values!"; warning_surf = self._render_text_safe(warning_text, "small_text", RED)
            warning_bg_box = pygame.Surface((warning_surf.get_width() + self.INSTRUCTION_PADDING_X, warning_surf.get_height() + self.INSTRUCTION_PADDING_Y), pygame.SRCALPHA)
            warning_bg_box.fill(self.INSTRUCTION_BG_COLOR) ; warning_bg_box.blit(warning_surf, warning_surf.get_rect(center=(warning_bg_box.get_width() // 2, warning_bg_box.get_height() // 2)))
            self.screen.blit(warning_bg_box, warning_bg_box.get_rect(center=(WIDTH // 2, self.SECONDARY_INSTRUCTION_CENTER_Y)))

    def draw_gameplay_hud(self):
        # (Logic remains the same, uses self.game_controller for player, score, lives, etc.)
        # (and self.drone_system for cores)
        ...
        h_padding = 20; v_padding = 10; element_spacing = 6; bar_height = 18; icon_to_bar_gap = 10; icon_spacing = 5; text_icon_spacing = 2
        current_time_ticks = pygame.time.get_ticks(); label_font = self.fonts["ui_text"]; value_font = self.fonts["ui_values"]
        emoji_general_font = self.fonts["ui_emoji_general"]; small_value_font = self.fonts.get("small_text")
        vitals_x_start = h_padding; current_vitals_y = panel_y_start + panel_height - v_padding; vitals_section_width = int(WIDTH / 3.2)
        min_bar_segment_width = 25; bar_segment_reduction_factor = 0.85; player_obj = self.game_controller.player
        max_icon_width_left = 0; temp_weapon_icon_surf = self._render_text_safe(WEAPON_MODE_ICONS.get(player_obj.current_weapon_mode, "💥"), "ui_emoji_small", ORANGE)
        max_icon_width_left = max(max_icon_width_left, temp_weapon_icon_surf.get_width())
        if self.ui_assets.get("current_drone_life_icon"): max_icon_width_left = max(max_icon_width_left, self.ui_assets["current_drone_life_icon"].get_width())
        ...
        if life_icon_surf:
            lives_y_pos = current_vitals_y - self.ui_icon_size_lives[1]; lives_draw_x = vitals_x_start
            for i in range(self.game_controller.lives): self.screen.blit(life_icon_surf, (lives_draw_x + i * (self.ui_icon_size_lives[0] + icon_spacing), lives_y_pos))
            current_vitals_y = lives_y_pos - element_spacing
        weapon_bar_y_pos = current_vitals_y - bar_height; weapon_icon_surf = temp_weapon_icon_surf
        ...
        charge_fill_pct = 0.0; weapon_ready_color = PLAYER_BULLET_COLOR; cooldown_duration = player_obj.current_shoot_cooldown
        time_since_last_shot = current_time_ticks - player_obj.last_shot_time
        if player_obj.current_weapon_mode == get_game_setting("WEAPON_MODE_HEATSEEKER") or player_obj.current_weapon_mode == get_game_setting("WEAPON_MODE_HEATSEEKER_PLUS_BULLETS"):
        ...
        if active_powerup_for_ui and (player_obj.shield_active or player_obj.speed_boost_active):
            ...
            if active_powerup_for_ui == "shield" and player_obj.shield_active:
                powerup_icon_char = "🛡️"; powerup_bar_fill_color = powerup_details_config.get("color", LIGHT_BLUE); remaining_time = player_obj.shield_end_time - current_time_ticks
                ...
                powerup_icon_char = "💨"; powerup_bar_fill_color = powerup_details_config.get("color", GREEN); remaining_time = player_obj.speed_boost_end_time - current_time_ticks
        ...
        cores_emoji_char = "💠"; cores_x_char = "x"; cores_value_str = str(self.drone_system.get_player_cores())
        cores_icon_surf = self._render_text_safe(cores_emoji_char, "ui_emoji_general", GOLD); cores_x_surf = self._render_text_safe(cores_x_char, "small_text", WHITE)
        cores_value_text_surf = self._render_text_safe(cores_value_str, "small_text", GOLD)
        total_cores_width = (cores_icon_surf.get_width() + text_icon_spacing + cores_x_surf.get_width() + text_icon_spacing + cores_value_text_surf.get_width())
        cores_start_x_draw = collectibles_x_anchor - total_cores_width; cores_display_max_height = max(cores_icon_surf.get_height(), cores_x_surf.get_height(), cores_value_text_surf.get_height())
        cores_y_pos = current_collectibles_y_right - cores_display_max_height; current_x_offset = cores_start_x_draw
        self.screen.blit(cores_icon_surf, (current_x_offset, cores_y_pos + (cores_display_max_height - cores_icon_surf.get_height()) // 2)); current_x_offset += cores_icon_surf.get_width() + text_icon_spacing
        self.screen.blit(cores_x_surf, (current_x_offset, cores_y_pos + (cores_display_max_height - cores_x_surf.get_height()) // 2)); current_x_offset += cores_x_surf.get_width() + text_icon_spacing
        ...
        fragment_icon_h = self.ui_icon_size_fragments[1]; fragment_y_pos_hud = current_collectibles_y_right - fragment_icon_h; fragment_display_order_ids = []
        if CORE_FRAGMENT_DETAILS:
            try: sorted_frag_keys = sorted([k for k in CORE_FRAGMENT_DETAILS.keys() if k != "fragment_vault_core"]); fragment_display_order_ids = [CORE_FRAGMENT_DETAILS[key]["id"] for key in sorted_frag_keys if "id" in CORE_FRAGMENT_DETAILS[key]]
            except Exception as e: print(f"UIManager: Error creating fragment display order: {e}. Using unsorted."); fragment_display_order_ids = [details["id"] for _, details in CORE_FRAGMENT_DETAILS.items() if details and "id" in details]
        displayable_fragment_ids = fragment_display_order_ids[:TOTAL_CORE_FRAGMENTS_NEEDED]
        if hasattr(self.game_controller, 'fragment_ui_target_positions'): self.game_controller.fragment_ui_target_positions.clear()
        if TOTAL_CORE_FRAGMENTS_NEEDED > 0 :
            ...
            for i in range(TOTAL_CORE_FRAGMENTS_NEEDED):
                ...
                if i < len(displayable_fragment_ids): frag_id_for_this_slot = displayable_fragment_ids[i]
                icon_to_draw = self.ui_assets["core_fragment_empty_icon"]
                if frag_id_for_this_slot and frag_id_for_this_slot in self.game_controller.hud_displayed_fragments: icon_to_draw = self.ui_assets["core_fragment_icons"].get(frag_id_for_this_slot, self.ui_assets["core_fragment_empty_icon"])
                current_frag_x = fragments_block_start_x + i * (self.ui_icon_size_fragments[0] + icon_spacing)
                if icon_to_draw: self.screen.blit(icon_to_draw, (current_frag_x, fragment_y_pos_hud))
                if frag_id_for_this_slot and hasattr(self.game_controller, 'fragment_ui_target_positions'): self.game_controller.fragment_ui_target_positions[frag_id_for_this_slot] = (current_frag_x + self.ui_icon_size_fragments[0] // 2, fragment_y_pos_hud + self.ui_icon_size_fragments[1] // 2)
            current_collectibles_y_right = fragment_y_pos_hud - element_spacing
        total_rings_this_level = self.game_controller.total_rings_per_level; displayed_rings_count = self.game_controller.displayed_collected_rings_count
        if self.ui_assets["ring_icon"] and total_rings_this_level > 0:
            ring_icon_h = self.ui_icon_size_rings[1]; rings_y_pos_hud = current_collectibles_y_right - ring_icon_h
            ...
            for i in range(total_rings_this_level):
                ...
                if icon_to_draw: self.screen.blit(icon_to_draw, (rings_block_start_x + i * (self.ui_icon_size_rings[0] + icon_spacing), rings_y_pos_hud))
        ...
        score_emoji_char = "🏆 "; score_text_str = f"Score: {self.game_controller.score}"; score_emoji_surf = self._render_text_safe(score_emoji_char, "ui_emoji_general", GOLD)
        score_text_surf = self._render_text_safe(score_text_str, "ui_text", GOLD); level_emoji_char = "🎯 "; level_text_str = f"Level: {self.game_controller.level}"
        ...
        level_emoji_surf = self._render_text_safe(level_emoji_char, "ui_emoji_general", CYAN); level_text_surf = self._render_text_safe(level_text_str, "ui_text", CYAN)
        time_icon_char = "⏱ "; time_ms_to_display = self.game_controller.level_time_remaining_ms
        ...
        time_seconds_total = max(0, time_ms_to_display // 1000); time_value_str = f"{time_seconds_total // 60:02d}:{time_seconds_total % 60:02d}"; time_color = WHITE
        is_vault_extraction = (current_scene_state.startswith("architect_vault") and self.game_controller.architect_vault_current_phase == "extraction")
        ...
        time_icon_surf = self._render_text_safe(time_icon_char, "ui_emoji_general", time_color); time_value_surf = self._render_text_safe(time_value_str, "ui_text", time_color)
        ...
        info_y_baseline = panel_y_start + (panel_height - max_central_element_height) // 2; spacing_between_center_elements = 25
        center_elements_total_width = (score_emoji_surf.get_width() + text_icon_spacing + score_text_surf.get_width() + spacing_between_center_elements + level_emoji_surf.get_width() + text_icon_spacing + level_text_surf.get_width())
        if not is_vault_extraction: center_elements_total_width += (spacing_between_center_elements + time_icon_surf.get_width() + text_icon_spacing + time_value_surf.get_width())
        current_info_x = (WIDTH - center_elements_total_width) // 2
        self.screen.blit(score_emoji_surf, (current_info_x, info_y_baseline + (max_central_element_height - score_emoji_surf.get_height()) // 2)); current_info_x += score_emoji_surf.get_width() + text_icon_spacing
        self.screen.blit(score_text_surf, (current_info_x, info_y_baseline + (max_central_element_height - score_text_surf.get_height()) // 2)); current_info_x += score_text_surf.get_width() + spacing_between_center_elements
        self.screen.blit(level_emoji_surf, (current_info_x, info_y_baseline + (max_central_element_height - level_emoji_surf.get_height()) // 2)); current_info_x += level_emoji_surf.get_width() + text_icon_spacing
        self.screen.blit(level_text_surf, (current_info_x, info_y_baseline + (max_central_element_height - level_text_surf.get_height()) // 2)); current_info_x += level_text_surf.get_width() + spacing_between_center_elements
        if not is_vault_extraction:
            self.screen.blit(time_icon_surf, (current_info_x, info_y_baseline + (max_central_element_height - time_icon_surf.get_height()) // 2)); current_info_x += time_icon_surf.get_width() + text_icon_spacing
        ...
        if hasattr(self.game_controller, 'animating_rings_to_hud'):
            for ring_anim in self.game_controller.animating_rings_to_hud:
                if 'surface' in ring_anim and ring_anim['surface']:
                    anim_surf = ring_anim['surface']; draw_x = int(ring_anim['pos'][0] - anim_surf.get_width() / 2); draw_y = int(ring_anim['pos'][1] - anim_surf.get_height() / 2)
        ...
        if hasattr(self.game_controller, 'animating_fragments_to_hud'):
            for frag_anim in self.game_controller.animating_fragments_to_hud:
                if 'surface' in frag_anim and frag_anim['surface']:
                    anim_surf = frag_anim['surface']; draw_x = int(frag_anim['pos'][0] - anim_surf.get_width() / 2); draw_y = int(frag_anim['pos'][1] - anim_surf.get_height() / 2)
                    self.screen.blit(anim_surf, (draw_x, draw_y))

    def get_scaled_fragment_icon(self, fragment_id):
        # (Logic remains the same)
        if not self.ui_assets["core_fragment_icons"] and not self.ui_assets["core_fragment_empty_icon"]: self._load_ui_assets()
        if fragment_id in self.ui_assets["core_fragment_icons"]: return self.ui_assets["core_fragment_icons"][fragment_id]
    ...
    def draw_architect_vault_hud_elements(self):
        # (Logic remains the same, uses self.game_controller for vault-specific data)
        self.draw_gameplay_hud() 
        current_time = pygame.time.get_ticks(); current_vault_phase = self.game_controller.architect_vault_current_phase
        if current_vault_phase == "extraction":
            time_remaining_ms_vault = self.game_controller.level_time_remaining_ms
            time_val_str_vault = f"{max(0, time_remaining_ms_vault // 1000) // 60:02d}:{max(0, time_remaining_ms_vault // 1000) % 60:02d}"; time_color_vault = RED
        ...
        vault_message = self.game_controller.architect_vault_message; vault_message_timer_end = self.game_controller.architect_vault_message_timer
        if vault_message and current_time < vault_message_timer_end:
            ...
            msg_bg_surf.fill((10, 0, 20, 200)); msg_bg_surf.blit(msg_surf, msg_surf.get_rect(center=(msg_bg_surf.get_width()//2, msg_bg_surf.get_height()//2)))
            self.screen.blit(msg_bg_surf, msg_bg_surf.get_rect(centerx=WIDTH//2, bottom=GAME_PLAY_AREA_HEIGHT - 20))

    def draw_pause_overlay(self):
        # (Logic remains the same)
        overlay_surface = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA); overlay_surface.fill((0,0,0,150)); self.screen.blit(overlay_surface, (0,0))
    ...
    def draw_game_over_overlay(self):
        # (Uses self.game_controller for score, level, and is_current_score_high helper)
        go_text_surf = self._render_text_safe("GAME OVER", "large_text", RED); score_text_surf = self._render_text_safe(f"Final Score: {self.game_controller.score}", "medium_text", WHITE)
        self.screen.blit(go_text_surf, go_text_surf.get_rect(center=(WIDTH//2, HEIGHT//2 - 120))); self.screen.blit(score_text_surf, score_text_surf.get_rect(center=(WIDTH//2, HEIGHT//2 - 30)))
        can_submit_score = not gs.SETTINGS_MODIFIED
        is_high = self.game_controller.is_current_score_a_high_score() # Assumes GameController has this helper
        ...
        if not can_submit_score:
            ...
            self.screen.blit(no_lb_text_surf, no_lb_text_surf.get_rect(center=(WIDTH//2, prompt_y_offset))); prompt_y_offset += self.fonts["ui_text"].get_height() + 20
            prompt_str = "R: Restart  M: Menu  Q: Quit"; prompt_color = WHITE
        elif is_high: prompt_str = "New High Score! Press any key to enter name."; prompt_color = GOLD
        else: prompt_str = "R: Restart  L: Leaderboard  M: Menu  Q: Quit"; prompt_color = WHITE
        prompt_surf = self._render_text_safe(prompt_str, "ui_text", prompt_color); self.screen.blit(prompt_surf, prompt_surf.get_rect(center=(WIDTH//2, prompt_y_offset)))

    def draw_enter_name_overlay(self):
        ui_flow_ctrl = self.game_controller.ui_flow_controller
        # (Logic remains the same, uses ui_flow_ctrl.player_name_input_cache and self.game_controller for score/level)
        title_surf = self._render_text_safe("New High Score!", "large_text", GOLD); self.screen.blit(title_surf, title_surf.get_rect(center=(WIDTH//2, HEIGHT//2 - 180)))
        score_level_text = f"Your Score: {self.game_controller.score} (Level: {self.game_controller.level})"; score_level_surf = self._render_text_safe(score_level_text, "medium_text", WHITE)
        self.screen.blit(score_level_surf, score_level_surf.get_rect(center=(WIDTH//2, HEIGHT//2 - 90)))
        prompt_name_surf = self._render_text_safe("Enter Name (max 6 chars, A-Z):", "ui_text", WHITE); self.screen.blit(prompt_name_surf, prompt_name_surf.get_rect(center=(WIDTH//2, HEIGHT//2 - 20)))
        ...
        pygame.draw.rect(self.screen, WHITE, input_box_rect, 2, border_radius=10); input_text_surf = self._render_text_safe(player_name_input_str, "input_text", WHITE)
        self.screen.blit(input_text_surf, input_text_surf.get_rect(center=input_box_rect.center)); submit_prompt_surf = self._render_text_safe("Press ENTER to submit.", "ui_text", CYAN)
    ...
    def draw_leaderboard_overlay(self):
        ui_flow_ctrl = self.game_controller.ui_flow_controller
        # (Logic remains the same, uses ui_flow_ctrl.leaderboard_scores)
        title_surf = self._render_text_safe("Leaderboard", "large_text", GOLD); title_bg_rect_width = title_surf.get_width() + 40; title_bg_rect_height = title_surf.get_height() + 20
        title_bg_surf = pygame.Surface((title_bg_rect_width, title_bg_rect_height), pygame.SRCALPHA); title_bg_surf.fill((20,20,20,180))
        title_bg_surf.blit(title_surf, title_surf.get_rect(center=(title_bg_rect_width//2, title_bg_rect_height//2))); self.screen.blit(title_bg_surf, title_bg_surf.get_rect(center=(WIDTH//2, HEIGHT//2 - 300)))
        scores_to_display = ui_flow_ctrl.leaderboard_scores; header_y = HEIGHT // 2 - 250; score_item_y_start = HEIGHT // 2 - 200
        item_line_height = self.fonts["leaderboard_entry"].get_height() + 15
        if not scores_to_display:
            no_scores_surf = self._render_text_safe("No scores yet!", "medium_text", WHITE); no_scores_bg = pygame.Surface((no_scores_surf.get_width()+20, no_scores_surf.get_height()+10), pygame.SRCALPHA)
            no_scores_bg.fill((30,30,30,160)); no_scores_bg.blit(no_scores_surf, no_scores_surf.get_rect(center=(no_scores_bg.get_width()//2, no_scores_bg.get_height()//2)))
            ...
            header_font = self.fonts.get("leaderboard_header", self.fonts["ui_text"]); entry_font = self.fonts.get("leaderboard_entry", self.fonts["ui_text"])
            for col_name, x_pos in cols_x_positions.items(): header_surf = header_font.render(col_name, True, WHITE); self.screen.blit(header_surf, (x_pos, header_y))
            for i, entry in enumerate(scores_to_display):
                ...
                texts_to_draw = [(f"{i+1}.", WHITE, cols_x_positions["Rank"]), (str(entry.get('name','N/A')).upper(), CYAN, cols_x_positions["Name"]), (str(entry.get('level','-')), GREEN, cols_x_positions["Level"]), (str(entry.get('score',0)), GOLD, cols_x_positions["Score"])]
                for text_str, color, x_coord in texts_to_draw: text_surf = entry_font.render(text_str, True, color); self.screen.blit(text_surf, (x_coord, y_pos))
        instr_text = "ESC: Main Menu | Q: Quit Game"; instr_surf = self._render_text_safe(instr_text, "ui_text", self.INSTRUCTION_TEXT_COLOR) 
        instr_bg_box = pygame.Surface((instr_surf.get_width() + self.INSTRUCTION_PADDING_X, instr_surf.get_height() + self.INSTRUCTION_PADDING_Y), pygame.SRCALPHA)
        instr_bg_box.fill(self.INSTRUCTION_BG_COLOR); instr_bg_box.blit(instr_surf, instr_surf.get_rect(center=(instr_bg_box.get_width() // 2, instr_bg_box.get_height() // 2)))
        self.screen.blit(instr_bg_box, instr_bg_box.get_rect(center=(WIDTH // 2, self.BOTTOM_INSTRUCTION_CENTER_Y)))
    ...
    def draw_architect_vault_success_overlay(self):
        ui_flow_ctrl = self.game_controller.ui_flow_controller
        # (Logic uses ui_flow_ctrl.architect_vault_result_message and _color)
        ...
        prompt_surf = self._render_text_safe("Press ENTER or M to Continue", "ui_text", WHITE); self.screen.blit(prompt_surf, prompt_surf.get_rect(center=(WIDTH//2, HEIGHT//2 + 100)))

    def draw_architect_vault_failure_overlay(self):
        ui_flow_ctrl = self.game_controller.ui_flow_controller
        # (Logic uses ui_flow_ctrl.architect_vault_result_message and _color)
        # (self.game_controller.architect_vault_failure_reason for specific reason text)
        msg_surf = self._render_text_safe(ui_flow_ctrl.architect_vault_result_message, "large_text", ui_flow_ctrl.architect_vault_result_message_color)
        self.screen.blit(msg_surf, msg_surf.get_rect(center=(WIDTH//2, HEIGHT//2 - 50)))
        # Display specific failure reason if available from GameController
    ...
    def draw_maze_defense_hud(self):
        # (Logic uses self.game_controller for player/score/lives, self.drone_system for cores)
        # (Wave info from self.game_controller.combat_controller.wave_manager)
        # (Reactor health from self.game_controller.combat_controller.core_reactor)
        panel_y_start = GAME_PLAY_AREA_HEIGHT; panel_height = BOTTOM_PANEL_HEIGHT
        panel_surf = pygame.Surface((WIDTH, panel_height), pygame.SRCALPHA); panel_surf.fill((20, 25, 35, 220)) 
        pygame.draw.line(panel_surf, (80, 120, 170, 200), (0, 0), (WIDTH, 0), 2); self.screen.blit(panel_surf, (0, panel_y_start))
        h_padding = 20; v_padding = 10; element_spacing = 6; text_icon_spacing = 2
        icon_spacing = 5 # DEFINED icon_spacing here

        if self.game_controller.player: # Player section
            vitals_x_start = h_padding; current_hud_y = panel_y_start + panel_height - v_padding 
            life_icon_surf = self.ui_assets.get("current_drone_life_icon")
            if life_icon_surf:
                lives_y_pos = current_hud_y - self.ui_icon_size_lives[1]
                for i in range(self.game_controller.lives): self.screen.blit(life_icon_surf, (vitals_x_start + i * (self.ui_icon_size_lives[0] + icon_spacing), lives_y_pos)) 
                current_hud_y = lives_y_pos - element_spacing
            cores_y_pos = current_hud_y - self.ui_icon_size_reactor[1]; cores_x_pos = vitals_x_start
            cores_emoji_char = "💠"; cores_value_str = str(self.drone_system.get_player_cores())
            cores_icon_surf = self._render_text_safe(cores_emoji_char, "ui_emoji_general", GOLD); cores_value_text_surf = self._render_text_safe(cores_value_str, "ui_values", GOLD)
            self.screen.blit(cores_icon_surf, (cores_x_pos, cores_y_pos + (self.ui_icon_size_reactor[1] - cores_icon_surf.get_height()) // 2))
            self.screen.blit(cores_value_text_surf, (cores_x_pos + cores_icon_surf.get_width() + text_icon_spacing, cores_y_pos + (self.ui_icon_size_reactor[1] - cores_value_text_surf.get_height()) // 2))
        
        center_hud_x = WIDTH // 2 # Defense mode specific info
        # Access wave_manager via combat_controller
        ...
        if combat_ctrl and combat_ctrl.wave_manager:
            ...
            wave_text_str = wave_manager.get_current_wave_display(); wave_surf = self._render_text_safe(wave_text_str, "ui_text", CYAN)
            ...
            if wave_manager.is_build_phase_active:
                build_time_str = wave_manager.get_build_phase_time_remaining_display(); build_time_surf = self._render_text_safe(build_time_str, "ui_text", YELLOW)
                ...
                turret_prompt_surf = self._render_text_safe("T: Place Turret", "small_text", GREEN); start_wave_prompt_surf = self._render_text_safe("SPACE: Start Wave", "small_text", GREEN)
                total_width_prompts = turret_prompt_surf.get_width() + start_wave_prompt_surf.get_width() + 20; start_x_prompts = center_hud_x - total_width_prompts // 2
                ...
                start_wave_prompt_rect = start_wave_prompt_surf.get_rect(left=turret_prompt_rect.right + 20, centery=turret_prompt_rect.centery); self.screen.blit(start_wave_prompt_surf, start_wave_prompt_rect)
            else: self.screen.blit(self._render_text_safe("Wave In Progress!", "ui_text", ORANGE), self._render_text_safe("Wave In Progress!", "ui_text", ORANGE).get_rect(centerx=center_hud_x, top=prompt_y_start))
        else: self.screen.blit(self._render_text_safe("Loading Defense...", "ui_text", GREY), self._render_text_safe("Loading Defense...", "ui_text", GREY).get_rect(centerx=center_hud_x, centery=panel_y_start + panel_height // 2))
        
        reactor = combat_ctrl.core_reactor if combat_ctrl else None # Reactor health bar
        if reactor and (reactor.alive or reactor.current_health > 0):
            ...
            health_percentage = reactor.current_health / reactor.max_health if reactor.max_health > 0 else 0; filled_width = bar_width * health_percentage
            ...
            if reactor_label_icon: self.screen.blit(reactor_label_icon, reactor_label_icon.get_rect(midright=(bar_x - 10, bar_y + bar_height_reactor // 2)))
        ...
        score_x_anchor = WIDTH - h_padding # Player Score
        score_emoji_char = "🏆 "; score_text_str = f"Score: {self.game_controller.score}"
        score_emoji_surf = self._render_text_safe(score_emoji_char, "ui_emoji_general", GOLD); score_text_surf = self._render_text_safe(score_text_str, "ui_text", GOLD)
        score_total_width = score_emoji_surf.get_width() + text_icon_spacing + score_text_surf.get_width(); score_start_x = score_x_anchor - score_total_width
