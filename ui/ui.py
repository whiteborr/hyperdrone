...
# Import the new game state for Maze Defense
if not hasattr(gs, 'GAME_STATE_MAZE_DEFENSE'):
    GAME_STATE_MAZE_DEFENSE = "maze_defense_mode" 
    print("UIManager: GAME_STATE_MAZE_DEFENSE not found in game_settings, using local fallback.")
else:
...
try:
    ...
    BuildMenu = None # Fallback if BuildMenu is not found
...
class UIManager:
    ...
    def draw_current_scene_ui(self):
        ...
        if current_state == GAME_STATE_MAIN_MENU: self.draw_main_menu()
            ...
            if self.game_controller.paused: self.draw_pause_overlay()
            # Draw BuildMenu if active and it's build phase
    ...
    def draw_game_intro_scroll(self): 
        ...
        current_image_surface = getattr(self.game_controller, 'current_intro_image_surface', None)
        current_text_surfaces = getattr(self.game_controller, 'intro_screen_text_surfaces_current', [])
        ...
        if current_image_surface:
            ...
            try:
                scaled_image = pygame.transform.smoothscale(current_image_surface, (scaled_w, scaled_h))
        ...
        if not current_text_surfaces:
            if hasattr(self.game_controller, 'intro_sequence_finished') and self.game_controller.intro_sequence_finished:
                prompt_font = self.fonts.get("small_text", pygame.font.Font(None, 24))
                prompt_text = "Press SPACE or ENTER to Continue"
                prompt_surf = prompt_font.render(prompt_text, True, CYAN)
                prompt_rect = prompt_surf.get_rect(centerx=WIDTH // 2, bottom=HEIGHT - 30)
        ...
        start_y = (HEIGHT - total_text_height) // 2; current_y = start_y
        fade_alpha = 255
        if hasattr(self.game_controller, 'intro_screen_start_time') and \
           hasattr(self.game_controller, 'INTRO_SCREEN_DURATION_MS'):
            elapsed_time = pygame.time.get_ticks() - self.game_controller.intro_screen_start_time
            fade_duration = 1000 
            if elapsed_time < fade_duration: fade_alpha = int(255 * (elapsed_time / fade_duration))
            elif self.game_controller.INTRO_SCREEN_DURATION_MS - elapsed_time < fade_duration:
                fade_alpha = int(255 * ((self.game_controller.INTRO_SCREEN_DURATION_MS - elapsed_time) / fade_duration))
        ...
        if current_view == "categories":
            ...
            if entry_data:
                ...
                if is_drone_entry and image_path:
                    ...
                    if cached_image:
                        img_max_width = list_panel_width - 20; img_max_height = HEIGHT * 0.3
                ...
                if is_race_entry and image_path:
                    ...
                    if cached_race_image:
                        img_max_width = content_panel_width * 0.6; img_max_height_race = HEIGHT * 0.25
    ...
    def draw_main_menu(self):
        ...
        if hasattr(self.game_controller, 'menu_stars') and self.game_controller.menu_stars:
            for star_params in self.game_controller.menu_stars: pygame.draw.circle(self.screen, WHITE, (int(star_params[0]), int(star_params[1])), star_params[3])
        selected_option_idx = getattr(self.game_controller, 'selected_menu_option', 0)
        menu_item_start_y = HEIGHT // 2 - 80 ; item_spacing = 85 
        unselected_label_font_size = 48; selected_label_font_size = 54
        ...
        min_button_width = 300 ; fixed_button_width = max(min_button_width, max_combined_width + horizontal_padding); fixed_button_height = max_content_height + vertical_padding
    ...
    def draw_drone_select_menu(self):
        ...
        drone_options_ids = getattr(self.game_controller, 'drone_select_options', DRONE_DISPLAY_ORDER); selected_preview_idx = getattr(self.game_controller, 'selected_drone_preview_index', 0)
        if not drone_options_ids: no_drones_surf = self._render_text_safe("NO DRONES AVAILABLE", "large_text", RED); self.screen.blit(no_drones_surf, no_drones_surf.get_rect(center=(WIDTH // 2, HEIGHT // 2))); return
        ...
        is_currently_equipped = (current_drone_id == self.drone_system.get_selected_drone_id()); drone_image_surf = None
        if hasattr(self.game_controller, 'drone_main_display_cache'): drone_image_surf = self.game_controller.drone_main_display_cache.get(current_drone_id)
    ...
    def draw_settings_menu(self):
        ...
        settings_items = getattr(self.game_controller, 'settings_items_data', []); selected_idx = getattr(self.game_controller, 'selected_setting_index', 0)
    ...
    def draw_gameplay_hud(self):
        ...
        if TOTAL_CORE_FRAGMENTS_NEEDED > 0 :
            ...
            for i in range(TOTAL_CORE_FRAGMENTS_NEEDED):
                ...
                if frag_id_for_this_slot: self.game_controller.fragment_ui_target_positions[frag_id_for_this_slot] = (current_frag_x + self.ui_icon_size_fragments[0] // 2, fragment_y_pos_hud + self.ui_icon_size_fragments[1] // 2)
            current_collectibles_y_right = fragment_y_pos_hud - element_spacing
        total_rings_this_level = getattr(self.game_controller, 'total_rings_per_level', 5); displayed_rings_count = getattr(self.game_controller, 'displayed_collected_rings', 0)
        ...
        is_vault_extraction = (current_scene_state.startswith("architect_vault") and getattr(self.game_controller, 'architect_vault_current_phase', None) == "extraction")
    ...
    def draw_architect_vault_hud_elements(self):
        self.draw_gameplay_hud() 
        current_time = pygame.time.get_ticks(); current_vault_phase = getattr(self.game_controller, 'architect_vault_current_phase', None)
        ...
        vault_message = getattr(self.game_controller, 'architect_vault_message', ""); vault_message_timer_end = getattr(self.game_controller, 'architect_vault_message_timer', 0)
    ...
    def draw_game_over_overlay(self):
        ...
        can_submit_score = not gs.SETTINGS_MODIFIED; is_high = leaderboard.is_high_score(self.game_controller.score, self.game_controller.level)
    ...
    def draw_enter_name_overlay(self):
        ...
        player_name_input_str = getattr(self.game_controller, 'player_name_input_display_cache', ""); input_box_width = 300; input_box_height = 60
    ...
    def draw_leaderboard_overlay(self):
        ...
        scores_to_display = getattr(self.game_controller, 'leaderboard_scores', []); header_y = HEIGHT // 2 - 250; score_item_y_start = HEIGHT // 2 - 200
    ...
    def draw_architect_vault_success_overlay(self):
        msg_surf = self._render_text_safe("Vault Conquered!", "large_text", GOLD); self.screen.blit(msg_surf, msg_surf.get_rect(center=(WIDTH//2, HEIGHT//2 - 80)))
        blueprint_id = get_game_setting("ARCHITECT_REWARD_BLUEPRINT_ID"); reward_text = f"Blueprint Acquired: {DRONE_DATA.get(blueprint_id, {}).get('name', blueprint_id)}" if blueprint_id else "Ancient Technology Secured!"
        reward_surf = self._render_text_safe(reward_text, "medium_text", CYAN); self.screen.blit(reward_surf, reward_surf.get_rect(center=(WIDTH//2, HEIGHT//2 + 20)))
    ...
    def draw_architect_vault_failure_overlay(self):
        msg_surf = self._render_text_safe("Vault Mission Failed", "large_text", RED); self.screen.blit(msg_surf, msg_surf.get_rect(center=(WIDTH//2, HEIGHT//2 - 50)))
        reason_text = getattr(self.game_controller, 'architect_vault_failure_reason', "Critical systems compromised."); reason_surf = self._render_text_safe(reason_text, "ui_text", YELLOW)
        self.screen.blit(reason_surf, reason_surf.get_rect(center=(WIDTH//2, HEIGHT//2 + 20))); prompt_surf = self._render_text_safe("Press ENTER or M to Return to Menu", "ui_text", WHITE)
        self.screen.blit(prompt_surf, prompt_surf.get_rect(center=(WIDTH//2, HEIGHT//2 + 80)))

    def draw_maze_defense_hud(self):
        """Draws the HUD specifically for the Maze Defense mode."""
        ...
        icon_spacing = 5; text_icon_spacing = 2
        label_font = self.fonts.get("ui_text", pygame.font.Font(None, 28))
        value_font = self.fonts.get("ui_values", pygame.font.Font(None, 30))
        small_font = self.fonts.get("small_text", pygame.font.Font(None, 24))
        emoji_font = self.fonts.get("ui_emoji_general", pygame.font.Font(None, 32))
        ...
        if self.game_controller.player:
            ...
            life_icon_surf = self.ui_assets.get("current_drone_life_icon")
            if life_icon_surf:
                lives_y_pos = current_hud_y - self.ui_icon_size_lives[1] # Align bottom of icons
            ...
            # Player Cores (Positioned next to lives or below)
            cores_y_pos = current_hud_y - self.ui_icon_size_reactor[1] # Align with other icons
            cores_x_pos = vitals_x_start # Below lives
            # If you want it next to lives:
            # cores_x_pos = vitals_x_start + (self.ui_icon_size_lives[0] + icon_spacing) * gs.get_game_setting("PLAYER_LIVES", 3) + 20
        ...
        if hasattr(self.game_controller, 'wave_manager') and self.game_controller.wave_manager:
            ...
            # Build Phase Timer / Status & Prompts
            ...
            if wave_manager.is_build_phase_active:
                ...
                combat_phase_rect = combat_phase_surf.get_rect(centerx=center_hud_x, top=prompt_y_start)
                self.screen.blit(combat_phase_surf, combat_phase_rect)
        else: # Fallback if wave_manager not ready
            loading_surf = self._render_text_safe("Loading Defense...", "ui_text", GREY)
