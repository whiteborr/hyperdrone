...
class UIManager: 
    def __init__(self, screen, fonts, game_controller_ref, scene_manager_ref, drone_system_ref): 
        ...
        # Ensure GameController has the attribute to store target positions
    ...
    def _load_ui_assets(self): 
        ...
        if CORE_FRAGMENT_DETAILS:
            for frag_key, details in CORE_FRAGMENT_DETAILS.items():
                ...
                elif frag_id and frag_id not in self.ui_assets["core_fragment_icons"]: # Fallback if details are incomplete
    ...
    def draw_main_menu(self): # Minimal implementation from original file
    ...
    def draw_drone_select_menu(self): # Minimal implementation from original file
    ...
    def draw_settings_menu(self): # Minimal implementation from original file
    ...
    def draw_gameplay_hud(self): 
        ...
        # Define a fixed order for fragment display if needed, e.g., based on typical collection order
        # This is an example, ensure it matches how your game logic expects fragment IDs.
        fragment_display_order_ids = []
        if CORE_FRAGMENT_DETAILS: # Check if it's populated
            # Attempt to sort by key (e.g., "fragment_alpha", "fragment_beta") for some consistency
            try:
            ...
            except Exception as e: # Fallback if sorting or access fails
        ...
        # Ensure we only try to display up to TOTAL_CORE_FRAGMENTS_NEEDED slots
        ...
        if hasattr(self.game_controller, 'fragment_ui_target_positions'):
            self.game_controller.fragment_ui_target_positions.clear() # Clear before repopulating

        if self.ui_assets["core_fragment_icons"] or self.ui_assets["core_fragment_empty_icon"]:
            total_fragments_width = len(displayable_fragment_ids) * (self.ui_icon_size_fragments[0] + icon_spacing)
            if len(displayable_fragment_ids) > 0 : total_fragments_width -= icon_spacing # Remove last spacing

            fragments_block_start_x = cores_start_x_draw - total_fragments_width - (icon_spacing * 4) # Gap before cores
            
            for i in range(TOTAL_CORE_FRAGMENTS_NEEDED): # Always draw slots for total needed
                ...
                is_collected_this_slot = False
                if frag_id_for_this_slot and self.drone_system.has_collected_fragment(frag_id_for_this_slot):
                    icon_to_draw = self.ui_assets["core_fragment_icons"].get(frag_id_for_this_slot, self.ui_assets["core_fragment_empty_icon"])
                    is_collected_this_slot = True
                ...
                # Store target position for animation, using the defined order
        ...
        # Rings Display (To the left of Fragments)
        ...
        if self.ui_assets["ring_icon"]: 
            ...
            rings_block_start_x = fragments_block_start_x - total_ring_icons_width_only - (icon_spacing * 4) if TOTAL_CORE_FRAGMENTS_NEEDED > 0 else \
                                  cores_start_x_draw - total_ring_icons_width_only - (icon_spacing * 4)
        ...
        # Info Section (Score, Level, Timer) - remains centered
    ...
    def get_scaled_fragment_icon(self, fragment_id):
        ...
        if not self.ui_assets["core_fragment_icons"] and not self.ui_assets["core_fragment_empty_icon"]:
            self._load_ui_assets() # Attempt to load if not already
