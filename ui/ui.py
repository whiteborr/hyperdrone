...
from game_settings import (
    ...
    GAME_STATE_GAME_INTRO_SCROLL, # This state will now represent the cinematic sequence
    DEFAULT_SETTINGS,
...
class UIManager:
    ...
    def draw_current_scene_ui(self):
        ...
        # GAME_STATE_GAME_INTRO_SCROLL is handled by its own draw method called from GameController.run()

        if is_menu_like_state: # Background for most menu screens
        ...
        if current_state == GAME_STATE_MAIN_MENU: self.draw_main_menu()
            ...
            if self.game_controller.paused: self.draw_pause_overlay()
        # Note: GAME_STATE_GAME_INTRO_SCROLL is handled by draw_game_intro_scroll, called directly in game_loop

        # Persistent story message display (if active)
        if hasattr(self.game_controller, 'story_message_active') and self.game_controller.story_message_active and \
           hasattr(self.game_controller, 'story_message') and self.game_controller.story_message:
            # Don't draw this if the intro scroll is active, as that's a full-screen message
            if current_state != GAME_STATE_GAME_INTRO_SCROLL:
                self.draw_story_message_overlay(self.game_controller.story_message)

    def draw_game_intro_scroll(self): # Corrected method name
        """Draws the current cinematic intro screen with background image and text."""
        ...
        if current_image_surface:
            ...
            pos_x = 0
            if scaled_h > HEIGHT: # If image is taller than screen after scaling to width
                scaled_h = HEIGHT
            ...
            try:
                ...
                # Fallback to stars if image fails
                if hasattr(self.game_controller, 'menu_stars') and self.game_controller.menu_stars:
                    for star_params in self.game_controller.menu_stars:
                        pygame.draw.circle(self.screen, WHITE, (int(star_params[0]), int(star_params[1])), star_params[3])
        else:
    ...
    def draw_story_message_overlay(self, message):
        ...
        box_y = GAME_PLAY_AREA_HEIGHT - box_height - 20 # Position above HUD
    ...
    def draw_codex_screen(self):
        ...
        if self.codex_list_item_height == 0 and current_list_item_height_val > 0 :
             ...
             if self.codex_list_item_height > 0:
                ...
                self.codex_max_visible_items_list = 1
