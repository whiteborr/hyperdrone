import math
import pygame
import random # For drawing jagged lightning effect
...
from game_settings import (
    ...
    LIGHTNING_COLOR as FALLBACK_LIGHTNING_COLOR 
)

# Fallback definitions for Lightning constants
_L_COLOR_FALLBACK = getattr(__import__('game_settings', fromlist=['LIGHTNING_COLOR']), 'LIGHTNING_COLOR', (0, 180, 255)) 
_L_DAMAGE_FALLBACK = 15
_L_LIFETIME_FALLBACK = 60 # Default visual duration in frames (increased as per user)
...
class LightningBullet(pygame.sprite.Sprite):
    def __init__(self, origin_pos, target_enemy,
                 damage=_L_DAMAGE_FALLBACK,
                 lifetime=_L_LIFETIME_FALLBACK, 
                 color=_L_COLOR_FALLBACK,
                 owner=None):
        super().__init__()
        self.origin_pos = pygame.math.Vector2(origin_pos)
        self.target_enemy = target_enemy 
        self.target_pos = pygame.math.Vector2(target_enemy.rect.center) if target_enemy else self.origin_pos 

        self.damage = damage
        self.lifetime = lifetime 
        self.initial_lifetime = max(1, lifetime) 
        self.color = color
        self.line_thickness = 5 # Increased for debugging visibility
        self.alive = True
        self.owner = owner

        # print(f"DEBUG: LightningBullet CREATED. Lifetime: {self.lifetime}, Target Enemy: {self.target_enemy is not None}, Origin: {self.origin_pos}, Target Pos: {self.target_pos}")

        if self.target_enemy and hasattr(self.target_enemy, 'take_damage') and self.target_enemy.alive:
            self.target_enemy.take_damage(self.damage)

        # Create an image and rect that encompasses the line for sprite group management
        # This helps ensure Pygame knows which area of the screen needs updating.
        min_x = min(self.origin_pos.x, self.target_pos.x)
        min_y = min(self.origin_pos.y, self.target_pos.y)
        max_x = max(self.origin_pos.x, self.target_pos.x)
        max_y = max(self.origin_pos.y, self.target_pos.y)
        
        width = max(1, max_x - min_x) # Ensure width is at least 1
        height = max(1, max_y - min_y) # Ensure height is at least 1

        self.image = pygame.Surface((width, height), pygame.SRCALPHA)
        self.rect = self.image.get_rect(topleft=(min_x, min_y))
        # The actual drawing will be on the main surface, relative to these points.
    ...
    def update(self):
        ...
        if self.lifetime <= 0:
            self.alive = False
            self.kill()
            # print(f"DEBUG: LightningBullet KILLED due to lifetime. ID: {id(self)}")
            return
        
        if self.target_enemy and self.target_enemy.alive:
            self.target_pos.update(self.target_enemy.rect.center)
            # Update rect if target moves significantly, though for a short-lived effect, this might be overkill
            # For simplicity, rect is set once in __init__ to cover the initial zap area.
            # If the zap needs to visually "follow" a fast-moving target perfectly, rect would need dynamic updates.
    ...
    def draw(self, surface):
        ...
        # Always draw a solid, thick, bright red line to ensure visibility
        debug_draw_color = (255, 0, 0) # BRIGHT RED
        debug_thickness = 5

        # Ensure target_pos is valid even if target_enemy is gone (use last known position)
        current_target_draw_pos = self.target_pos
        if self.target_enemy and self.target_enemy.alive:
             current_target_draw_pos = pygame.math.Vector2(self.target_enemy.rect.center)


        print(f"DEBUG: LightningBullet DRAWING. ID: {id(self)}, Lifetime: {self.lifetime}, "
              f"From: ({self.origin_pos.x:.1f}, {self.origin_pos.y:.1f}), "
              f"To: ({current_target_draw_pos.x:.1f}, {current_target_draw_pos.y:.1f}), "
              f"Color: {debug_draw_color}, Thickness: {debug_thickness}")

        try:
            if self.origin_pos.distance_to(current_target_draw_pos) > 1: # Avoid drawing a dot
                 pygame.draw.line(surface, debug_draw_color, self.origin_pos, current_target_draw_pos, debug_thickness)
            else: # If points are too close, draw a small circle at origin to show something
                 pygame.draw.circle(surface, debug_draw_color, (int(self.origin_pos.x), int(self.origin_pos.y)), debug_thickness)
        except Exception as e:
            print(f"ERROR in LightningBullet.draw() during pygame.draw.line: {e}")
        
        # --- Original Jagged Line & Alpha Fade Logic (Commented out for debugging) ---
        # current_alpha = int(255 * (self.lifetime / self.initial_lifetime))
        # current_alpha = max(0, min(255, current_alpha))
        # if current_alpha == 0: return
        # points = [self.origin_pos]
        # num_segments = random.randint(4, 7)
        # if self.origin_pos.distance_to(self.target_pos) < 5:
        #      try:
        #         line_color_with_alpha = (*self.color[:3], current_alpha)
        #         pygame.draw.line(surface, line_color_with_alpha, self.origin_pos, self.target_pos, self.line_thickness)
        #      except ValueError:
        #          temp_line_surf = pygame.Surface(surface.get_size(), pygame.SRCALPHA)
        #          pygame.draw.line(temp_line_surf, self.color, self.origin_pos, self.target_pos, self.line_thickness)
        #          temp_line_surf.set_alpha(current_alpha)
        #          surface.blit(temp_line_surf, (0,0))
        #      return
        # dx_segment = (self.target_pos.x - self.origin_pos.x) / num_segments
        # dy_segment = (self.target_pos.y - self.origin_pos.y) / num_segments
