...
import random
import math
# import traceback # Removed for debugging
...
from entities import (
    ...
    CoreReactor,
    Turret,
    LightningZap, # Ensure LightningZap is imported
    Missile,      # Ensure Missile is imported
    Particle,
    MazeGuardian,
    SentinelDrone,
    EscapeZone,
...
class GameController:
    ...
    def _create_fallback_image_surface(self, size=(200,200), text="?", color=(80,80,80), text_color=WHITE, font_key="large_text"):
        ...
        if text and font_to_use:
            try:
                ...
                pass # Error rendering fallback text
        return surface
        ...
        if beat_id not in self.triggered_story_beats:
            if self.drone_system.has_unlocked_lore(beat_id):
                ...
                if lore_entry:
                    ...
                    pass # Story Beat details not found
            else:
                pass # Story Beat lore not unlocked
        return False
                   ...
                   not self.ring_puzzle_has_been_officially_handled_as_solved:
                    ...
                    if self.last_interacted_terminal and hasattr(self.last_interacted_terminal, 'item_id'):
                        ...
                        if self.drone_system.mark_puzzle_terminal_as_solved(terminal_id_solved):
                            pass # Terminal marked as solved
    ...
    def _update_maze_defense_state(self, current_time_ms, delta_time_ms):
        if not self.player or not self.maze or not self.core_reactor:
            self.scene_manager.set_game_state(GAME_STATE_MAIN_MENU)
            return
            ...
            for bullet, enemies_hit in hit_enemies_by_bullets.items():
                ...
                for enemy in enemies_hit:
                    if enemy.alive and bullet.rect.colliderect(enemy.collision_rect):
                        ...
                        if not bullet.alive: break # Bullet is done

            # Missiles
            hit_enemies_by_missiles = pygame.sprite.groupcollide(
                turret_obj.missiles, self.enemy_manager.get_sprites(),
                True, False, pygame.sprite.collide_rect_ratio(0.7) # Missiles are dokill=True
            )
            for missile, enemies_hit in hit_enemies_by_missiles.items():
                for enemy in enemies_hit: # A missile typically hits one primary target
                    if enemy.alive: # Ensure enemy is still alive (could be hit by another projectile simultaneously)
                        enemy.take_damage(missile.damage)
                        self._create_explosion(missile.rect.centerx, missile.rect.centery, num_particles=15, specific_sound='prototype_drone_explode') # Missile explosion
                        if not enemy.alive:
                            self.score += 40; self.drone_system.add_player_cores(20)
                            # Enemy explosion already handled if take_damage kills it
                        break # Missile hits one target
                    
            # Lightning Zaps
            for zap in list(turret_obj.lightning_zaps): # Iterate copy as zaps might kill themselves
                ...
                for enemy_hit in enemies_hit_by_zap:
                    if enemy_hit.alive :
                        enemy_hit.take_damage(zap.damage)
                        ...
                        break # Lightning zap hits one target then damage_applied is true

        # --- End Turret Projectile Collision ---
        ...
        if not self.level_cleared_pending_animation:
            ...
            for ring_sprite in collided_rings_sprites:
                ...
                if hasattr(ring_sprite, 'image') and self.ui_manager.ui_assets.get("ring_icon"):
                    ...
                    except Exception as e: pass # Error scaling ring
                if anim_ring_surf:
            ...
            if hasattr(log_item, 'apply_effect') and log_item.apply_effect(self.player, self):
                ...
                if log_id and self.drone_system:
                    ...
                    if unlocked_lore:
                        ...
                        pass # Log collected, no new lore
            ...
            if hasattr(tablet_item, 'apply_effect') and tablet_item.apply_effect(self.player, self):
                ...
                if tablet_id and self.drone_system:
                    ...
                    if message_to_show:
                        ...
                        pass # Tablet collected, no new specific lore
    ...
    def _check_collisions_architect_vault_combat(self):
                    ...
                    if target_obj.alive:
                        ...
                        if not target_obj.alive:
                            if target_obj is self.maze_guardian:
                                self.score += 1000; self.drone_system.add_player_cores(1000)
        ...
        if self.boss_active and self.maze_guardian:
            if hasattr(self.maze_guardian, 'bullets') and self.maze_guardian.bullets:
        ...
        for projectile_obj in list(all_enemy_projectiles):
            if self.player.alive and projectile_obj.rect.colliderect(self.player.collision_rect):
                ...
                if isinstance(projectile_obj, LightningZap):
                    damage_amount = gs.get_game_setting("MAZE_GUARDIAN_LASER_DAMAGE")
            ...
            for entity_obj in physical_collisions:
                if entity_obj.alive:
                    ...
                    if entity_obj is self.maze_guardian:
                        damage_on_collision = gs.get_game_setting("MAZE_GUARDIAN_LASER_DAMAGE") * 1.5
    ...
    def _handle_player_death_or_life_loss(self, reason=""):
        ...
        if self.lives <= 0:
            ...
            current_game_state = self.scene_manager.get_current_state()
            if current_game_state == GAME_STATE_PLAYING or current_game_state == GAME_STATE_BONUS_LEVEL_PLAYING:
                self._reset_level_timer_internal()
    ...
    def _prepare_for_next_level(self, from_bonus_level_completion=False):
        ...
        if not from_bonus_level_completion:
            ...
            pass # Returning to level after bonus
    ...
    def _get_safe_spawn_point(self, entity_width, entity_height):
        ...
        if not path_cells_abs:
             return (getattr(self.maze, 'game_area_x_offset', 0) + TILE_SIZE // 2, TILE_SIZE // 2)
        first_abs_x, first_abs_y = random.choice(path_cells_abs)
    ...
    def get_enemy_spawn_points_for_defense(self):
        if isinstance(self.maze, MazeChapter2):
            ...
            if spawn_points_abs:
                ...
                pass # MazeChapter2 has no enemy spawn points
    ...
    def _spawn_architect_vault_terminals(self):
        ...
        if len(path_cells_abs) < TOTAL_CORE_FRAGMENTS_NEEDED:
            return
            ...
            if not self.drone_system.has_puzzle_terminal_been_solved(terminal_unique_id):
                ...
                if not already_spawned:
                    ...
                    if available_path_tiles_for_terminal_abs:
                        ...
                        if not spawned_terminal: pass # Could not find suitable spawn
                    else: pass # No path cells to spawn
            else:
                pass # Terminal already solved
    ...
    def _get_random_valid_fragment_tile_internal_abs(self, existing_fragment_tiles_abs):
        ...
        available_path_tiles_abs = [
            (abs_x, abs_y) for abs_x, abs_y in path_cells_abs
            if (abs_x, abs_y) not in existing_fragment_tiles_abs
        ]
        ...
        if self.codex_current_view == "categories":
            ...
            if key_event == pygame.K_UP:
                ...
                if self.codex_current_entry_total_lines > 0:
                    max_visible = self.ui_manager.codex_max_visible_lines_content if self.ui_manager.codex_max_visible_lines_content > 0 else 1
    ...
    def try_place_turret(self, screen_pos):
        ...
        if self.player.rect.collidepoint(turret_center_x, turret_center_y):
            self.play_sound('ui_denied', 0.6)
            return False


        if self.drone_system.get_player_cores() >= Turret.TURRET_COST:
