            ...
            if event.type == pygame.KEYDOWN: 
                ...
                if current_game_state == GAME_STATE_MAIN_MENU: 
                    ...
                    if hasattr(self.game_controller, 'handle_codex_input'):
                        self.game_controller.handle_codex_input(event.key) # Pass the key to GameController
                    # Fallback for ESC if handle_codex_input doesn't consume it or isn't fully implemented yet
                    elif event.key == pygame.K_ESCAPE: 
                        if hasattr(self.game_controller, 'play_sound'): self.game_controller.play_sound('ui_select')
                        # Default ESC action in Codex might be handled by game_controller.handle_codex_input now
                        # For safety, if not handled there, go to main menu.
