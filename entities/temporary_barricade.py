...
class TemporaryBarricade(Sprite):
    ...
    def _create_barricade_image(self):
        ...
        rect(barricade_surface, outer_color, (0, 0, self.size, self.size), border_radius=5)
        ...
        rect(barricade_surface, inner_color, inner_rect, border_radius=3)

        # Cross beams
        line_color = WHITE
        line_thickness = max(1, int(self.size * 0.05))
        line(barricade_surface, line_color, (self.size * 0.2, self.size * 0.2), (self.size * 0.8, self.size * 0.8), line_thickness)
        line(barricade_surface, line_color, (self.size * 0.8, self.size * 0.2), (self.size * 0.2, self.size * 0.8), line_thickness)
    ...
    def update(self):
        ...
        if current_time - self.spawn_time > self.lifetime_ms:
            self.alive = False
            self.kill()
            debug(f"Barricade at ({self.x:.1f}, {self.y:.1f}) expired.")
            return
    ...
    def _draw_health_bar(self, surface, camera=None):
        ...
        rect(surface, (80, 0, 0), (bar_x, bar_y, bar_w, bar_h))
        
        if fill_w > 0:
            rect(surface, fill_color, (bar_x, bar_y, int(fill_w), bar_h))
            
        rect(surface, WHITE, (bar_x, bar_y, bar_w, bar_h), 1)
