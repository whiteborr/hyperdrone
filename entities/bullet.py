...
class Bullet(pygame.sprite.Sprite):
    ...
    def update(self, maze=None, game_area_x_offset=0):
        if not self.alive:
            # self.kill() # DEBUG: Temporarily commented out to see if draw() is called on dead bullets
            return
        ...
        if self.lifetime <= 0:
            ...
            self.alive = False
            # self.kill() # DEBUG: Temporarily commented out to keep in group for draw test
            return 
        
        FORCE_DISABLE_WALL_COLLISION_DEBUG = True 

        if not self.can_pierce_walls and not FORCE_DISABLE_WALL_COLLISION_DEBUG: 
            # This block should be skipped if FORCE_DISABLE_WALL_COLLISION_DEBUG is True
            # print(f"CRITICAL DEBUG Bullet: EXECUTING WALL COLLISION LOGIC! pierce={self.can_pierce_walls}, FORCE_DISABLE={FORCE_DISABLE_WALL_COLLISION_DEBUG}")
            wall_hit = False
            if maze:
            ...
            if wall_hit:
                if self.bounces_done < self.max_bounces:
                    ...
                    if not bounced_this_frame and (hit_x_wall and hit_y_wall): 
                        self.dx *= -1; self.dy *= -1; self.angle = (self.angle + 180) % 360
                    self.bounces_done += 1
                    self.x += self.dx * 0.1; self.y += self.dy * 0.1
                    self.rect.center = (int(self.x), int(self.y))
                else: 
                    ...
                    self.alive = False
                    # self.kill() # DEBUG: Temporarily commented out
                return 
        ...
        if not (min_x_bound < self.rect.centerx < max_x_bound and min_y_bound < self.rect.centery < max_y_bound):
            if self.bounces_done < self.max_bounces: 
                ...
                if self.rect.top < min_y_bound or self.rect.bottom > max_y_bound:
                    self.dy *= -1; self.angle = (-self.angle) % 360 if not bounced_on_boundary else (self.angle + 180 + (-2 * self.angle)) % 360; bounced_on_boundary = True 
    ...
    def draw(self, surface): 
        # This print will now always execute when Bullet.draw() is called by the sprite group.
