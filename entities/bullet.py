...
class Missile(Sprite): 
    def __init__(self, x, y, initial_angle, damage, enemies_group):
        ...
        missile_w, missile_h = missile_size*1.5, missile_size*3; self.original_image_surface = Surface([missile_w, missile_h], SRCALPHA)
        ...
        polygon(self.original_image_surface, (255, 200, 0), [(missile_w*0.5, missile_h*0.95), (missile_w*0.35, missile_h*0.8), (missile_w*0.65, missile_h*0.8)])
        # Draw white rocket body
        from pygame import Rect as PygameRect
        body_rect = PygameRect(missile_w*0.25, missile_h*0.15, missile_w*0.5, missile_h*0.65)
        ...
        self.original_image = rotate(self.original_image_surface,-90); self.image = self.original_image
    ...
    def _attempt_slide(self, maze, next_x_direct, next_y_direct, direct_rad_angle): 
        ...
        for da_deg in [30, 60, 45, 75, 90]: 
            for sign in [1, -1]: 
                ...
                s_dx, s_dy = cos(angle_rad) * self.speed, sin(angle_rad) * self.speed
                if not maze.is_wall(self.x+s_dx, self.y+s_dy, collision_width,collision_height) and \
                   not maze.is_wall(self.x+s_dx*1.5, self.y+s_dy*1.5, collision_width,collision_height):
                    self.angle = degrees(atan2(s_dy, s_dx)); return s_dx, s_dy
        return None
    ...
    def update(self, enemies_group_updated=None, maze=None, game_area_x_offset=0, shmup_mode=False): 
        ...
        if self.target: 
            ...
            if dx_to_target != 0 or dy_to_target != 0: self.target_angle = degrees(atan2(dy_to_target, dx_to_target))
...
class LightningZap(Sprite):
    def __init__(self, player_ref, initial_target_enemy_ref, damage, lifetime_frames, maze_ref, game_area_x_offset=0, color_override=None, enemies_group=None):
        ...
        self.chain_targets = []  # List of chained targets
    ...
    def _calculate_potential_target_pos(self, ignore_snapshot=False): 
        ...
        return (start_x + cos(angle_rad) * lightning_zap_range, start_y + sin(angle_rad) * lightning_zap_range)
    ...
    def _update_rect_for_collision(self): 
        ...
        from pygame import Rect as PygameRect
        self.rect = PygameRect(int(min_x-padding), int(min_y-padding), int(max_x-min_x+2*padding), int(max_y-min_y+2*padding))
    ...
    def _build_chain(self):
        ...
        while current_target and current_target not in visited:
            # Skip boss enemies
            if hasattr(current_target, 'enemy_type') and 'boss' in current_target.enemy_type.lower():
                break
            if hasattr(current_target, '__class__') and 'guardian' in current_target.__class__.__name__.lower():
                break
    ...
    def update(self, current_time_ms): 
        ...
        # Apply damage to all chain targets on first frame
    ...
    def draw(self, surface, camera=None):
        if self.alive:
            ...
            if alpha > 5:
                # Draw main lightning bolt from player to first target
                if self.chain_targets and self.chain_targets[0].alive:
                    self._draw_lightning_bolt(surface, self.current_start_pos, self.chain_targets[0].rect.center, self.color, alpha)
                else:
                    # Fallback to original target position if no chain targets
                ...
                for i in range(len(self.chain_targets) - 1):
                    if self.chain_targets[i].alive and self.chain_targets[i+1].alive:
                        start_pos = self.chain_targets[i].rect.center
                        end_pos = self.chain_targets[i+1].rect.center
                        self._draw_lightning_bolt(surface, start_pos, end_pos, self.color, alpha // 2)
